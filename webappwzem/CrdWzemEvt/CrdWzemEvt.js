/**
  * \file CrdWzemEvt.js
  * web client functionality for card CrdWzemEvt
...
function getInitdone(pnlshort) {
	return(retrieveSi(srcdoc, "StatAppWzemEvt", "initdone" + pnlshort) == "true");
};

function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppWzemEvt", "initdone" + pnlshort, "true");

	checkInitdone();
};

function checkInitdone() {
	var initdoneHeadbar = (retrieveSi(srcdoc, "StatAppWzemEvt", "initdoneHeadbar") == "true");
	var initdoneList = (retrieveSi(srcdoc, "StatAppWzemEvt", "initdoneList") == "true");
	var initdoneRec = (retrieveSi(srcdoc, "StatAppWzemEvt", "initdoneRec") == "true");

	if (!initdoneHeadbar) {
		doc.getElementById("Headbar").src = "./PnlWzemEvtHeadbar.html?scrJref=" + scrJrefHeadbar;
	} else if (!initdoneList) {
		doc.getElementById("List").src = "./PnlWzemEvtList.html?scrJref=" + scrJrefList;
	} else if (!initdoneRec) {
		doc.getElementById("Rec").src = "./PnlWzemEvtRec.html?scrJref=" + scrJrefRec;
	} else {
		doc.initdone = true;
...
function toggleMenu(short, left, width, height) {
	var shortMenu = retrieveSi(srcdoc, "StatAppWzemEvt", "shortMenu");
...
function showMenu(short, left, width, height) {
	...
	setSi(srcdoc, "StatAppWzemEvt", "shortMenu", short);
	setSi(srcdoc, "StatAppWzemEvt", "widthMenu", "" + (width-3));
...
function hideMenu(short) {
	...
	setSi(srcdoc, "StatAppWzemEvt", "shortMenu", "");
...
function initMenApp() {
	...
	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppWzemEvt", "widthMenu"));

	setTextContent(mendoc, mendoc.getElementById("MitAppAbt"), retrieveTi(srcdoc, "TagWzemEvt", "MitAppAbt"));
	setTextContent(mendoc, mendoc.getElementById("MrlAppHlp"), retrieveTi(srcdoc, "TagWzemEvt", "MrlAppHlp"));
};
...
function initMenCrd() {
	...
	setTextContent(mendoc, mendoc.getElementById("MtxCrdEvt"), retrieveCi(srcdoc, "ContInfWzemEvt", "MtxCrdEvt"));
};
...
function refresh() {
	...
	doc.title = retrieveCi(srcdoc, "ContInfWzemEvt", "MtxCrdEvt") + " - WhizniumSBE Engine Monitor v0.9.19";
};
...
function handleUnload() {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzemEvtDo", scrJref, "close");
	sendReq(str, doc, null, true);
};

function handleMitClick(menshort, consref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzemEvtDo", scrJref, consref + "Click");
	sendReq(str, doc, handleDpchAppDoReply);
...
function handleMrlClick(menshort, consref) {
	window.open(retrieveCi(srcdoc, "ContInfWzemEvt", consref), "_blank");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWzemEvtData", "ContInfWzemEvt", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWzemEvtData", "FeedFSge", srcdoc)) mask.push("feedFSge");
	if (updateSrcblock(dom, "DpchEngWzemEvtData", "StatAppWzemEvt", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWzemEvtData", "StatShrWzemEvt", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWzemEvtData", "TagWzemEvt", srcdoc)) mask.push("tag");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWzemEvtData") {
				mergeDpchEngData(dom);

				// establish sub-panel scrJref's as global variables, and start sub-panel load sequence
				scrJrefHeadbar = retrieveSi(srcdoc, "StatShrWzemEvt", "scrJrefHeadbar");
				scrJrefList = retrieveSi(srcdoc, "StatShrWzemEvt", "scrJrefList");
				scrJrefRec = retrieveSi(srcdoc, "StatShrWzemEvt", "scrJrefRec");
		...
		if (blk) {
			...
			if (_scrJref != "") {
				if (_scrJref == scrJref) {
					...
					} else if (blk.nodeName == "DpchEngWzemEvtData") {
						mergeDpchEngData(dom);
						refresh();
					};
...
function restartNotify() {
	var srefIxWzemVReqitmode = retrieveSi(srcdoc, "StatAppWzemEvt", "srefIxWzemVReqitmode");

	if (srefIxWzemVReqitmode != "notify") {
		if (doc.timeout) window.clearTimeout(doc.timeout);

		srefIxWzemVReqitmode = "notify";
		setSi(srcdoc, "StatAppWzemEvt", "srefIxWzemVReqitmode", srefIxWzemVReqitmode);
...
function iterateReqit(ev) {
	...
	var srefIxWzemVReqitmode = retrieveSi(srcdoc, "StatAppWzemEvt", "srefIxWzemVReqitmode");
	var latency = parseInt(retrieveSi(srcdoc, "StatAppWzemEvt", "latency"));
