...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlWzemEvtList_aside.html";
	doc.getElementById("List_cont").src = "./PnlWzemEvtList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlWzemEvtList_bside.html";
	doc.getElementById("List_cont").src = "./PnlWzemEvtList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzemEvtList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWzemEvtList", "Trs"));
...
function init() {
	var srefIxWzemVExpstate = retrieveSi(srcdoc, "StatShrWzemEvtList", "srefIxWzemVExpstate");
...
function refreshA() {
	var TxtFor = retrieveCi(srcdoc, "ContInfWzemEvtList", "TxtFor");

	setTextContent(contdoc, contdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWzemEvtList", "TxtFor"), TxtFor));

	if (TxtFor == "") {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzemEvtList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWzemEvtList", "ntot") + ")");
		clearElem(contdoc.getElementById("TxtPre"));
	} else {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzemEvtList", "Cpt"));
		setTextContent(contdoc, contdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWzemEvtList", "TxtPre") + " (" + retrieveSi(srcdoc, "StatShrQryWzemEvtList", "ntot") + ")");
	};
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWzemEvtList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemEvtList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemEvtList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemEvtList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemEvtList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWzemEvtList", "ntot"));
	...
	// header row
	setTextContent(hdrdoc, hdrdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWzemEvtList", "TxtFor"), TxtFor));

	if (TxtFor == "") clearElem(hdrdoc.getElementById("TxtPre"));
	else setTextContent(hdrdoc, hdrdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWzemEvtList", "TxtPre"));

	// update column characteristics
	if (chgcol) {
		// obtain column (header) characteristics
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemEvtList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemEvtList", "ncol"));

		var TcoTypWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemEvtList", "TcoTypWidth"));
		var TcoPrdWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemEvtList", "TcoPrdWidth"));
		var TcoStaWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemEvtList", "TcoStaWidth"));
		var TcoCmdWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemEvtList", "TcoCmdWidth"));
		var TcoFegWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemEvtList", "TcoFegWidth"));
		var TcoMtdWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemEvtList", "TcoMtdWidth"));
		var TcoXsrWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemEvtList", "TcoXsrWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacWzemEvtList", "numFTos"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoPrd"); doc.widths.push(TcoPrdWidth); doc.fs.push("prd"); doc.tos.push(2); wtot += TcoPrdWidth;
			} else if (i == 3) {
				doc.cols.push("TcoSta"); doc.widths.push(TcoStaWidth); doc.fs.push("sta"); doc.tos.push(1); wtot += TcoStaWidth;
			} else if (i == 4) {
				doc.cols.push("TcoCmd"); doc.widths.push(TcoCmdWidth); doc.fs.push("cmd"); doc.tos.push(0); wtot += TcoCmdWidth;
			} else if (i == 5) {
				doc.cols.push("TcoFeg"); doc.widths.push(TcoFegWidth); doc.fs.push("feg"); doc.tos.push(0); wtot += TcoFegWidth;
			} else if (i == 6) {
				doc.cols.push("TcoMtd"); doc.widths.push(TcoMtdWidth); doc.fs.push("mtd"); doc.tos.push(0); wtot += TcoMtdWidth;
			} else if (i == 7) {
				doc.cols.push("TcoXsr"); doc.widths.push(TcoXsrWidth); doc.fs.push("xsr"); doc.tos.push(0); wtot += TcoXsrWidth;
			} else {
				ix--;
			};
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagWzemEvtList", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWzemEvtList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWzemQEvtList
		rownode = getRownode(srcdoc, "ListWzemQEvtList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWzemEvtList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWzemEvtList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWzemEvtList", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzemEvtList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemEvtList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemEvtList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWzemEvtList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWzemEvtList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWzemEvtListData", scrJref, "StgIacQryWzemEvtList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemEvtList", "firstcol"));

	setSi(srcdoc, "StatAppQryWzemEvtList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzemEvtList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzemEvtListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacWzemEvtList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacWzemEvtList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppWzemEvtListData", scrJref, "ContIacWzemEvtList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWzemEvtListData", "ContIacWzemEvtList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngWzemEvtListData", "ContInfWzemEvtList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWzemEvtListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWzemEvtListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngWzemEvtListData", "StatShrWzemEvtList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWzemEvtListData", "StgIacWzemEvtList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWzemEvtListData", "TagWzemEvtList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWzemEvtListData", "ListWzemQEvtList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWzemEvtListData", "StatAppQryWzemEvtList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWzemEvtListData", "StatShrQryWzemEvtList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWzemEvtListData", "StgIacQryWzemEvtList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngWzemEvtListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWzemEvtListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWzemEvtListData") {
				mask = mergeDpchEngData(dom);
