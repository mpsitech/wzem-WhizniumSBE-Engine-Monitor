/**
  * \file CrdWzemNde.js
  * web client functionality for card CrdWzemNde
...
function getInitdone(pnlshort) {
	return(retrieveSi(srcdoc, "StatAppWzemNde", "initdone" + pnlshort) == "true");
};

function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppWzemNde", "initdone" + pnlshort, "true");

	checkInitdone();
};

function checkInitdone() {
	var initdoneHeadbar = (retrieveSi(srcdoc, "StatAppWzemNde", "initdoneHeadbar") == "true");
	var initdoneList = (retrieveSi(srcdoc, "StatAppWzemNde", "initdoneList") == "true");
	var initdoneRec = (retrieveSi(srcdoc, "StatAppWzemNde", "initdoneRec") == "true");

	if (!initdoneHeadbar) {
		doc.getElementById("Headbar").src = "./PnlWzemNdeHeadbar.html?scrJref=" + scrJrefHeadbar;
	} else if (!initdoneList) {
		doc.getElementById("List").src = "./PnlWzemNdeList.html?scrJref=" + scrJrefList;
	} else if (!initdoneRec) {
		doc.getElementById("Rec").src = "./PnlWzemNdeRec.html?scrJref=" + scrJrefRec;
	} else {
		doc.initdone = true;
...
function toggleMenu(short, left, width, height) {
	var shortMenu = retrieveSi(srcdoc, "StatAppWzemNde", "shortMenu");
...
function showMenu(short, left, width, height) {
	...
	setSi(srcdoc, "StatAppWzemNde", "shortMenu", short);
	setSi(srcdoc, "StatAppWzemNde", "widthMenu", "" + (width-3));
...
function hideMenu(short) {
	...
	setSi(srcdoc, "StatAppWzemNde", "shortMenu", "");
...
function initMenApp() {
	...
	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppWzemNde", "widthMenu"));

	setTextContent(mendoc, mendoc.getElementById("MitAppAbt"), retrieveTi(srcdoc, "TagWzemNde", "MitAppAbt"));
	setTextContent(mendoc, mendoc.getElementById("MrlAppHlp"), retrieveTi(srcdoc, "TagWzemNde", "MrlAppHlp"));
};
...
function initMenCrd() {
	...
	setTextContent(mendoc, mendoc.getElementById("MtxCrdNde"), retrieveCi(srcdoc, "ContInfWzemNde", "MtxCrdNde"));
};
...
function refresh() {
	...
	doc.title = retrieveCi(srcdoc, "ContInfWzemNde", "MtxCrdNde") + " - WhizniumSBE Engine Monitor v0.9.19";
};
...
function handleUnload() {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzemNdeDo", scrJref, "close");
	sendReq(str, doc, null, true);
};

function handleMitClick(menshort, consref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzemNdeDo", scrJref, consref + "Click");
	sendReq(str, doc, handleDpchAppDoReply);
...
function handleMrlClick(menshort, consref) {
	window.open(retrieveCi(srcdoc, "ContInfWzemNde", consref), "_blank");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWzemNdeData", "ContInfWzemNde", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWzemNdeData", "FeedFSge", srcdoc)) mask.push("feedFSge");
	if (updateSrcblock(dom, "DpchEngWzemNdeData", "StatAppWzemNde", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWzemNdeData", "StatShrWzemNde", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWzemNdeData", "TagWzemNde", srcdoc)) mask.push("tag");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWzemNdeData") {
				mergeDpchEngData(dom);

				// establish sub-panel scrJref's as global variables, and start sub-panel load sequence
				scrJrefHeadbar = retrieveSi(srcdoc, "StatShrWzemNde", "scrJrefHeadbar");
				scrJrefList = retrieveSi(srcdoc, "StatShrWzemNde", "scrJrefList");
				scrJrefRec = retrieveSi(srcdoc, "StatShrWzemNde", "scrJrefRec");
		...
		if (blk) {
			...
			if (_scrJref != "") {
				if (_scrJref == scrJref) {
					...
					} else if (blk.nodeName == "DpchEngWzemNdeData") {
						mergeDpchEngData(dom);
						refresh();
					};
...
function restartNotify() {
	var srefIxWzemVReqitmode = retrieveSi(srcdoc, "StatAppWzemNde", "srefIxWzemVReqitmode");

	if (srefIxWzemVReqitmode != "notify") {
		if (doc.timeout) window.clearTimeout(doc.timeout);

		srefIxWzemVReqitmode = "notify";
		setSi(srcdoc, "StatAppWzemNde", "srefIxWzemVReqitmode", srefIxWzemVReqitmode);
...
function iterateReqit(ev) {
	...
	var srefIxWzemVReqitmode = retrieveSi(srcdoc, "StatAppWzemNde", "srefIxWzemVReqitmode");
	var latency = parseInt(retrieveSi(srcdoc, "StatAppWzemNde", "latency"));
