...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlWzemClnList_aside.html";
	doc.getElementById("List_cont").src = "./PnlWzemClnList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlWzemClnList_bside.html";
	doc.getElementById("List_cont").src = "./PnlWzemClnList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzemClnList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWzemClnList", "Trs"));
...
function init() {
	var srefIxWzemVExpstate = retrieveSi(srcdoc, "StatShrWzemClnList", "srefIxWzemVExpstate");
...
function refreshA() {
	var TxtFor = retrieveCi(srcdoc, "ContInfWzemClnList", "TxtFor");

	setTextContent(contdoc, contdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWzemClnList", "TxtFor"), TxtFor));

	if (TxtFor == "") {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzemClnList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWzemClnList", "ntot") + ")");
		clearElem(contdoc.getElementById("TxtPre"));
	} else {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzemClnList", "Cpt"));
		setTextContent(contdoc, contdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWzemClnList", "TxtPre") + " (" + retrieveSi(srcdoc, "StatShrQryWzemClnList", "ntot") + ")");
	};
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWzemClnList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemClnList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemClnList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemClnList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemClnList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWzemClnList", "ntot"));
	...
	// header row
	setTextContent(hdrdoc, hdrdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWzemClnList", "TxtFor"), TxtFor));

	if (TxtFor == "") clearElem(hdrdoc.getElementById("TxtPre"));
	else setTextContent(hdrdoc, hdrdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWzemClnList", "TxtPre"));

	// update column characteristics
	if (chgcol) {
		// obtain column (header) characteristics
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemClnList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemClnList", "ncol"));

		var TcoJobWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemClnList", "TcoJobWidth"));
		var TcoCalWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemClnList", "TcoCalWidth"));
		var TcoSgeWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemClnList", "TcoSgeWidth"));
		var TcoStaWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemClnList", "TcoStaWidth"));
		var TcoStoWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemClnList", "TcoStoWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacWzemClnList", "numFTos"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagWzemClnList", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWzemClnList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWzemQClnList
		rownode = getRownode(srcdoc, "ListWzemQClnList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWzemClnList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWzemClnList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWzemClnList", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzemClnList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemClnList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemClnList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWzemClnList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWzemClnList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWzemClnListData", scrJref, "StgIacQryWzemClnList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemClnList", "firstcol"));

	setSi(srcdoc, "StatAppQryWzemClnList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzemClnList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzemClnListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacWzemClnList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacWzemClnList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppWzemClnListData", scrJref, "ContIacWzemClnList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWzemClnListData", "ContIacWzemClnList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngWzemClnListData", "ContInfWzemClnList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWzemClnListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWzemClnListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngWzemClnListData", "StatShrWzemClnList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWzemClnListData", "StgIacWzemClnList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWzemClnListData", "TagWzemClnList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWzemClnListData", "ListWzemQClnList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWzemClnListData", "StatAppQryWzemClnList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWzemClnListData", "StatShrQryWzemClnList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWzemClnListData", "StgIacQryWzemClnList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngWzemClnListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWzemClnListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWzemClnListData") {
				mask = mergeDpchEngData(dom);
