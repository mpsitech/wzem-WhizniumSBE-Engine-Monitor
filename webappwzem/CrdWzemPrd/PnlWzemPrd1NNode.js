/**
  * \file PnlWzemPrd1NNode.js
  * web client functionality for panel PnlWzemPrd1NNode
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWzemPrd1NNode", "srefIxWzemVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWzemPrd1NNode", "srefIxWzemVExpstate", "mind");

	getRecwnd().changeHeight("1NNode", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("1NNode_side").setAttribute("height", "30");
	doc.getElementById("1NNode_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("1NNode_side").src = "./PnlWzemPrd1NNode_aside.html";
	doc.getElementById("1NNode_cont").src = "./PnlWzemPrd1NNode_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWzemPrd1NNode", "srefIxWzemVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWzemPrd1NNode", "srefIxWzemVExpstate", "regd");

	getRecwnd().changeHeight("1NNode", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("1NNode_side").setAttribute("height", "255");
	doc.getElementById("1NNode_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("1NNode_side").src = "./PnlWzemPrd1NNode_bside.html";
	doc.getElementById("1NNode_cont").src = "./PnlWzemPrd1NNode_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("1NNode_side").contentDocument;
	contdoc = doc.getElementById("1NNode_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("1NNode_side").contentDocument;
	contdoc = doc.getElementById("1NNode_cont").contentDocument;
	hdrdoc = contdoc.getElementById("1NNode_hdr").contentDocument;
	tbldoc = contdoc.getElementById("1NNode_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("1NNode_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzemPrd1NNode", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWzemPrd1NNode", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWzemVExpstate = retrieveSi(srcdoc, "StatAppWzemPrd1NNode", "srefIxWzemVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzemPrd1NNode", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWzemPrd1NNode", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWzemPrd1NNode", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemPrd1NNode", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemPrd1NNode", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemPrd1NNode", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemPrd1NNode", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWzemPrd1NNode", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemPrd1NNode", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemPrd1NNode", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemPrd1NNode", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWzemPrd1NNode", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWzemPrd1NNode", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWzemQPrd1NNode
		rownode = getRownode(srcdoc, "ListWzemQPrd1NNode", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWzemPrd1NNode", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWzemPrd1NNode", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWzemPrd1NNode", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("1NNode", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("1NNode_side").setAttribute("height", "" + (height+55));
	doc.getElementById("1NNode_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("1NNode_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("1NNode_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WzemPrd1NNode", "ListWzemQPrd1NNode", retrieveTi(srcdoc, "TagWzemPrd1NNode", "TxtRecord1"), retrieveTi(srcdoc, "TagWzemPrd1NNode", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzemPrd1NNode", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemPrd1NNode", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemPrd1NNode", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWzemPrd1NNode", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWzemPrd1NNode", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWzemPrd1NNodeData", scrJref, "StgIacQryWzemPrd1NNode");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWzemPrd1NNode", "srefIxWzemVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemPrd1NNode", "firstcol"));

	setSi(srcdoc, "StatAppQryWzemPrd1NNode", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWzemPrd1NNode", "srefIxWzemVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzemPrd1NNode", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzemPrd1NNodeDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWzemPrd1NNodeData", "ContInfWzemPrd1NNode", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWzemPrd1NNodeData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWzemPrd1NNodeData", "StatAppWzemPrd1NNode", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWzemPrd1NNodeData", "StatShrWzemPrd1NNode", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWzemPrd1NNodeData", "StgIacWzemPrd1NNode", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWzemPrd1NNodeData", "TagWzemPrd1NNode", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWzemPrd1NNodeData", "ListWzemQPrd1NNode", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWzemPrd1NNodeData", "StatAppQryWzemPrd1NNode", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWzemPrd1NNodeData", "StatShrQryWzemPrd1NNode", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWzemPrd1NNodeData", "StgIacQryWzemPrd1NNode", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWzemPrd1NNodeData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWzemPrd1NNodeData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("1NNode");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWzemPrd1NNodeData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
