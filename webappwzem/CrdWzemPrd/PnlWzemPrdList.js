/**
  * \file PnlWzemPrdList.js
  * web client functionality for panel PnlWzemPrdList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlWzemPrdList_aside.html";
	doc.getElementById("List_cont").src = "./PnlWzemPrdList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlWzemPrdList_bside.html";
	doc.getElementById("List_cont").src = "./PnlWzemPrdList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzemPrdList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWzemPrdList", "Trs"));
...
function init() {
	var srefIxWzemVExpstate = retrieveSi(srcdoc, "StatShrWzemPrdList", "srefIxWzemVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzemPrdList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWzemPrdList", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWzemPrdList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemPrdList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemPrdList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemPrdList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemPrdList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWzemPrdList", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemPrdList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemPrdList", "ncol"));

		var TcoGrpWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemPrdList", "TcoGrpWidth"));
		var TcoOwnWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemPrdList", "TcoOwnWidth"));
		var TcoVerWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemPrdList", "TcoVerWidth"));
		var TcoStaWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemPrdList", "TcoStaWidth"));
		var TcoStoWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemPrdList", "TcoStoWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacWzemPrdList", "numFTos"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoSto"); doc.widths.push(TcoStoWidth); doc.fs.push("sto"); doc.tos.push(0); wtot += TcoStoWidth;
			} else {
				ix--;
			};
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagWzemPrdList", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWzemPrdList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWzemQPrdList
		rownode = getRownode(srcdoc, "ListWzemQPrdList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWzemPrdList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWzemPrdList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWzemPrdList", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WzemPrdList", "ListWzemQPrdList", retrieveTi(srcdoc, "TagWzemPrdList", "TxtRecord1"), retrieveTi(srcdoc, "TagWzemPrdList", "TxtRecord2"),
				["Grp","Own","Ver","Sta","Sto"],
				["grp","own","ver","sta","sto"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzemPrdList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemPrdList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemPrdList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWzemPrdList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWzemPrdList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWzemPrdListData", scrJref, "StgIacQryWzemPrdList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemPrdList", "firstcol"));

	setSi(srcdoc, "StatAppQryWzemPrdList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzemPrdList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzemPrdListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacWzemPrdList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacWzemPrdList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppWzemPrdListData", scrJref, "ContIacWzemPrdList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWzemPrdListData", "ContIacWzemPrdList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngWzemPrdListData", "ContInfWzemPrdList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWzemPrdListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWzemPrdListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngWzemPrdListData", "StatShrWzemPrdList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWzemPrdListData", "StgIacWzemPrdList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWzemPrdListData", "TagWzemPrdList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWzemPrdListData", "ListWzemQPrdList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWzemPrdListData", "StatAppQryWzemPrdList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWzemPrdListData", "StatShrQryWzemPrdList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWzemPrdListData", "StgIacQryWzemPrdList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngWzemPrdListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWzemPrdListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWzemPrdListData") {
				mask = mergeDpchEngData(dom);
