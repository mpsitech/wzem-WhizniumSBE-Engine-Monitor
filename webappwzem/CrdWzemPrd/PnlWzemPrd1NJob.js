/**
  * \file PnlWzemPrd1NJob.js
  * web client functionality for panel PnlWzemPrd1NJob
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWzemPrd1NJob", "srefIxWzemVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWzemPrd1NJob", "srefIxWzemVExpstate", "mind");
	...
	// change content
	doc.getElementById("1NJob_side").src = "./PnlWzemPrd1NJob_aside.html";
	doc.getElementById("1NJob_cont").src = "./PnlWzemPrd1NJob_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWzemPrd1NJob", "srefIxWzemVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWzemPrd1NJob", "srefIxWzemVExpstate", "regd");
	...
	// change content
	doc.getElementById("1NJob_side").src = "./PnlWzemPrd1NJob_bside.html";
	doc.getElementById("1NJob_cont").src = "./PnlWzemPrd1NJob_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzemPrd1NJob", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWzemPrd1NJob", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWzemVExpstate = retrieveSi(srcdoc, "StatAppWzemPrd1NJob", "srefIxWzemVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzemPrd1NJob", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWzemPrd1NJob", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWzemPrd1NJob", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemPrd1NJob", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemPrd1NJob", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemPrd1NJob", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemPrd1NJob", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWzemPrd1NJob", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemPrd1NJob", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemPrd1NJob", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemPrd1NJob", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWzemPrd1NJob", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWzemPrd1NJob", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWzemQPrd1NJob
		rownode = getRownode(srcdoc, "ListWzemQPrd1NJob", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWzemPrd1NJob", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWzemPrd1NJob", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWzemPrd1NJob", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WzemPrd1NJob", "ListWzemQPrd1NJob", retrieveTi(srcdoc, "TagWzemPrd1NJob", "TxtRecord1"), retrieveTi(srcdoc, "TagWzemPrd1NJob", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzemPrd1NJob", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemPrd1NJob", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemPrd1NJob", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWzemPrd1NJob", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWzemPrd1NJob", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWzemPrd1NJobData", scrJref, "StgIacQryWzemPrd1NJob");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWzemPrd1NJob", "srefIxWzemVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemPrd1NJob", "firstcol"));

	setSi(srcdoc, "StatAppQryWzemPrd1NJob", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWzemPrd1NJob", "srefIxWzemVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzemPrd1NJob", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzemPrd1NJobDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWzemPrd1NJobData", "ContInfWzemPrd1NJob", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWzemPrd1NJobData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWzemPrd1NJobData", "StatAppWzemPrd1NJob", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWzemPrd1NJobData", "StatShrWzemPrd1NJob", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWzemPrd1NJobData", "StgIacWzemPrd1NJob", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWzemPrd1NJobData", "TagWzemPrd1NJob", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWzemPrd1NJobData", "ListWzemQPrd1NJob", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWzemPrd1NJobData", "StatAppQryWzemPrd1NJob", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWzemPrd1NJobData", "StatShrQryWzemPrd1NJob", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWzemPrd1NJobData", "StgIacQryWzemPrd1NJob", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWzemPrd1NJobData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWzemPrd1NJobData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWzemPrd1NJobData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
