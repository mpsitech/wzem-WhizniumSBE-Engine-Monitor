/**
  * \file PnlWzemPrd1NEvent.js
  * web client functionality for panel PnlWzemPrd1NEvent
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWzemPrd1NEvent", "srefIxWzemVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWzemPrd1NEvent", "srefIxWzemVExpstate", "mind");
	...
	// change content
	doc.getElementById("1NEvent_side").src = "./PnlWzemPrd1NEvent_aside.html";
	doc.getElementById("1NEvent_cont").src = "./PnlWzemPrd1NEvent_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWzemPrd1NEvent", "srefIxWzemVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWzemPrd1NEvent", "srefIxWzemVExpstate", "regd");
	...
	// change content
	doc.getElementById("1NEvent_side").src = "./PnlWzemPrd1NEvent_bside.html";
	doc.getElementById("1NEvent_cont").src = "./PnlWzemPrd1NEvent_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzemPrd1NEvent", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWzemPrd1NEvent", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWzemVExpstate = retrieveSi(srcdoc, "StatAppWzemPrd1NEvent", "srefIxWzemVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzemPrd1NEvent", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWzemPrd1NEvent", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWzemPrd1NEvent", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemPrd1NEvent", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemPrd1NEvent", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemPrd1NEvent", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemPrd1NEvent", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWzemPrd1NEvent", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemPrd1NEvent", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemPrd1NEvent", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemPrd1NEvent", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWzemPrd1NEvent", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWzemPrd1NEvent", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWzemQPrd1NEvent
		rownode = getRownode(srcdoc, "ListWzemQPrd1NEvent", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWzemPrd1NEvent", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWzemPrd1NEvent", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWzemPrd1NEvent", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WzemPrd1NEvent", "ListWzemQPrd1NEvent", retrieveTi(srcdoc, "TagWzemPrd1NEvent", "TxtRecord1"), retrieveTi(srcdoc, "TagWzemPrd1NEvent", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzemPrd1NEvent", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemPrd1NEvent", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemPrd1NEvent", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWzemPrd1NEvent", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWzemPrd1NEvent", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWzemPrd1NEventData", scrJref, "StgIacQryWzemPrd1NEvent");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWzemPrd1NEvent", "srefIxWzemVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemPrd1NEvent", "firstcol"));

	setSi(srcdoc, "StatAppQryWzemPrd1NEvent", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWzemPrd1NEvent", "srefIxWzemVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzemPrd1NEvent", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzemPrd1NEventDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWzemPrd1NEventData", "ContInfWzemPrd1NEvent", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWzemPrd1NEventData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWzemPrd1NEventData", "StatAppWzemPrd1NEvent", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWzemPrd1NEventData", "StatShrWzemPrd1NEvent", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWzemPrd1NEventData", "StgIacWzemPrd1NEvent", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWzemPrd1NEventData", "TagWzemPrd1NEvent", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWzemPrd1NEventData", "ListWzemQPrd1NEvent", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWzemPrd1NEventData", "StatAppQryWzemPrd1NEvent", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWzemPrd1NEventData", "StatShrQryWzemPrd1NEvent", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWzemPrd1NEventData", "StgIacQryWzemPrd1NEvent", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWzemPrd1NEventData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWzemPrd1NEventData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWzemPrd1NEventData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
