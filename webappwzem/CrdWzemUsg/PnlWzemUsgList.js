...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlWzemUsgList_aside.html";
	doc.getElementById("List_cont").src = "./PnlWzemUsgList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlWzemUsgList_bside.html";
	doc.getElementById("List_cont").src = "./PnlWzemUsgList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzemUsgList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWzemUsgList", "Trs"));
...
function init() {
	var srefIxWzemVExpstate = retrieveSi(srcdoc, "StatShrWzemUsgList", "srefIxWzemVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzemUsgList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWzemUsgList", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWzemUsgList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemUsgList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemUsgList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemUsgList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemUsgList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWzemUsgList", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemUsgList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemUsgList", "ncol"));

		var TcoGrpWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemUsgList", "TcoGrpWidth"));
		var TcoOwnWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemUsgList", "TcoOwnWidth"));
		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemUsgList", "TcoSrfWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacWzemUsgList", "numFTos"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagWzemUsgList", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWzemUsgList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWzemQUsgList
		rownode = getRownode(srcdoc, "ListWzemQUsgList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWzemUsgList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWzemUsgList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWzemUsgList", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzemUsgList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemUsgList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemUsgList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWzemUsgList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWzemUsgList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWzemUsgListData", scrJref, "StgIacQryWzemUsgList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemUsgList", "firstcol"));

	setSi(srcdoc, "StatAppQryWzemUsgList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzemUsgList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzemUsgListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacWzemUsgList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacWzemUsgList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppWzemUsgListData", scrJref, "ContIacWzemUsgList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWzemUsgListData", "ContIacWzemUsgList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngWzemUsgListData", "ContInfWzemUsgList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWzemUsgListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWzemUsgListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngWzemUsgListData", "StatShrWzemUsgList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWzemUsgListData", "StgIacWzemUsgList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWzemUsgListData", "TagWzemUsgList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWzemUsgListData", "ListWzemQUsgList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWzemUsgListData", "StatAppQryWzemUsgList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWzemUsgListData", "StatShrQryWzemUsgList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWzemUsgListData", "StgIacQryWzemUsgList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngWzemUsgListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWzemUsgListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWzemUsgListData") {
				mask = mergeDpchEngData(dom);
