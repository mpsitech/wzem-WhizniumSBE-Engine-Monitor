...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWzemUsgAAccess", "srefIxWzemVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWzemUsgAAccess", "srefIxWzemVExpstate", "mind");
	...
	// change content
	doc.getElementById("AAccess_side").src = "./PnlWzemUsgAAccess_aside.html";
	doc.getElementById("AAccess_cont").src = "./PnlWzemUsgAAccess_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWzemUsgAAccess", "srefIxWzemVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWzemUsgAAccess", "srefIxWzemVExpstate", "regd");
	...
	// change content
	doc.getElementById("AAccess_side").src = "./PnlWzemUsgAAccess_bside.html";
	doc.getElementById("AAccess_cont").src = "./PnlWzemUsgAAccess_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzemUsgAAccess", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWzemUsgAAccess", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWzemVExpstate = retrieveSi(srcdoc, "StatAppWzemUsgAAccess", "srefIxWzemVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzemUsgAAccess", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWzemUsgAAccess", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWzemUsgAAccess", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemUsgAAccess", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemUsgAAccess", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemUsgAAccess", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemUsgAAccess", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWzemUsgAAccess", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemUsgAAccess", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemUsgAAccess", "ncol"));

		var TcoFegWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemUsgAAccess", "TcoFegWidth"));
		var TcoFeaWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemUsgAAccess", "TcoFeaWidth"));
		var TcoAccWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemUsgAAccess", "TcoAccWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWzemUsgAAccess", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWzemUsgAAccess", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWzemQUsgAAccess
		rownode = getRownode(srcdoc, "ListWzemQUsgAAccess", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWzemUsgAAccess", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWzemUsgAAccess", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWzemUsgAAccess", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzemUsgAAccess", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemUsgAAccess", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemUsgAAccess", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWzemUsgAAccess", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWzemUsgAAccess", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWzemUsgAAccessData", scrJref, "StgIacQryWzemUsgAAccess");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWzemUsgAAccess", "srefIxWzemVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemUsgAAccess", "firstcol"));

	setSi(srcdoc, "StatAppQryWzemUsgAAccess", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWzemUsgAAccess", "srefIxWzemVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzemUsgAAccess", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzemUsgAAccessDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWzemUsgAAccessData", "ContInfWzemUsgAAccess", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWzemUsgAAccessData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWzemUsgAAccessData", "StatAppWzemUsgAAccess", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWzemUsgAAccessData", "StatShrWzemUsgAAccess", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWzemUsgAAccessData", "StgIacWzemUsgAAccess", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWzemUsgAAccessData", "TagWzemUsgAAccess", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWzemUsgAAccessData", "ListWzemQUsgAAccess", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWzemUsgAAccessData", "StatAppQryWzemUsgAAccess", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWzemUsgAAccessData", "StatShrQryWzemUsgAAccess", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWzemUsgAAccessData", "StgIacQryWzemUsgAAccess", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWzemUsgAAccessData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWzemUsgAAccessData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWzemUsgAAccessData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
