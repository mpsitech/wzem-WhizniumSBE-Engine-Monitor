...
function minimize(updh) {
	...
	setSi(srcdoc, "StatAppWzemUsgMNUser", "srefIxWzemVExpstate", "mind");
	...
	// change content
	doc.getElementById("MNUser_side").src = "./PnlWzemUsgMNUser_aside.html";
	doc.getElementById("MNUser_cont").src = "./PnlWzemUsgMNUser_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWzemUsgMNUser", "srefIxWzemVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWzemUsgMNUser", "srefIxWzemVExpstate", "regd");
	...
	// change content
	doc.getElementById("MNUser_side").src = "./PnlWzemUsgMNUser_bside.html";
	doc.getElementById("MNUser_cont").src = "./PnlWzemUsgMNUser_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzemUsgMNUser", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWzemUsgMNUser", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWzemVExpstate = retrieveSi(srcdoc, "StatAppWzemUsgMNUser", "srefIxWzemVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzemUsgMNUser", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWzemUsgMNUser", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWzemUsgMNUser", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemUsgMNUser", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemUsgMNUser", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemUsgMNUser", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemUsgMNUser", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWzemUsgMNUser", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemUsgMNUser", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemUsgMNUser", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemUsgMNUser", "TcoMrefWidth"));
		var TcoUlvWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemUsgMNUser", "TcoUlvWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWzemUsgMNUser", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWzemUsgMNUser", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWzemQUsgMNUser
		rownode = getRownode(srcdoc, "ListWzemQUsgMNUser", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWzemUsgMNUser", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWzemUsgMNUser", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWzemUsgMNUser", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WzemUsgMNUser", "ListWzemQUsgMNUser", retrieveTi(srcdoc, "TagWzemUsgMNUser", "TxtRecord1"), retrieveTi(srcdoc, "TagWzemUsgMNUser", "TxtRecord2"),
				["Mref","Ulv",""],
				["mref","ulv","ulv2"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzemUsgMNUser", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemUsgMNUser", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemUsgMNUser", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWzemUsgMNUser", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWzemUsgMNUser", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWzemUsgMNUserData", scrJref, "StgIacQryWzemUsgMNUser");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWzemUsgMNUser", "srefIxWzemVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemUsgMNUser", "firstcol"));

	setSi(srcdoc, "StatAppQryWzemUsgMNUser", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWzemUsgMNUser", "srefIxWzemVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzemUsgMNUser", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzemUsgMNUserDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWzemUsgMNUserData", "ContInfWzemUsgMNUser", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWzemUsgMNUserData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWzemUsgMNUserData", "StatAppWzemUsgMNUser", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWzemUsgMNUserData", "StatShrWzemUsgMNUser", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWzemUsgMNUserData", "StgIacWzemUsgMNUser", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWzemUsgMNUserData", "TagWzemUsgMNUser", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWzemUsgMNUserData", "ListWzemQUsgMNUser", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWzemUsgMNUserData", "StatAppQryWzemUsgMNUser", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWzemUsgMNUserData", "StatShrQryWzemUsgMNUser", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWzemUsgMNUserData", "StgIacQryWzemUsgMNUser", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWzemUsgMNUserData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWzemUsgMNUserData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWzemUsgMNUserData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
