/**
  * \file PnlWzemPrsList.js
  * web client functionality for panel PnlWzemPrsList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlWzemPrsList_aside.html";
	doc.getElementById("List_cont").src = "./PnlWzemPrsList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlWzemPrsList_bside.html";
	doc.getElementById("List_cont").src = "./PnlWzemPrsList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzemPrsList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWzemPrsList", "Trs"));
...
function init() {
	var srefIxWzemVExpstate = retrieveSi(srcdoc, "StatShrWzemPrsList", "srefIxWzemVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzemPrsList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWzemPrsList", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWzemPrsList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemPrsList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemPrsList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemPrsList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemPrsList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWzemPrsList", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemPrsList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemPrsList", "ncol"));

		var TcoGrpWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemPrsList", "TcoGrpWidth"));
		var TcoOwnWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemPrsList", "TcoOwnWidth"));
		var TcoTitWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemPrsList", "TcoTitWidth"));
		var TcoFnmWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemPrsList", "TcoFnmWidth"));
		var TcoLnmWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemPrsList", "TcoLnmWidth"));
		var TcoSexWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemPrsList", "TcoSexWidth"));
		var TcoTelWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemPrsList", "TcoTelWidth"));
		var TcoEmlWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemPrsList", "TcoEmlWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacWzemPrsList", "numFTos"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagWzemPrsList", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWzemPrsList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWzemQPrsList
		rownode = getRownode(srcdoc, "ListWzemQPrsList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWzemPrsList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWzemPrsList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWzemPrsList", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WzemPrsList", "ListWzemQPrsList", retrieveTi(srcdoc, "TagWzemPrsList", "TxtRecord1"), retrieveTi(srcdoc, "TagWzemPrsList", "TxtRecord2"),
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzemPrsList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemPrsList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemPrsList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWzemPrsList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWzemPrsList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWzemPrsListData", scrJref, "StgIacQryWzemPrsList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemPrsList", "firstcol"));

	setSi(srcdoc, "StatAppQryWzemPrsList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzemPrsList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzemPrsListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacWzemPrsList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacWzemPrsList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppWzemPrsListData", scrJref, "ContIacWzemPrsList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWzemPrsListData", "ContIacWzemPrsList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngWzemPrsListData", "ContInfWzemPrsList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWzemPrsListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWzemPrsListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngWzemPrsListData", "StatShrWzemPrsList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWzemPrsListData", "StgIacWzemPrsList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWzemPrsListData", "TagWzemPrsList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWzemPrsListData", "ListWzemQPrsList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWzemPrsListData", "StatAppQryWzemPrsList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWzemPrsListData", "StatShrQryWzemPrsList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWzemPrsListData", "StgIacQryWzemPrsList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngWzemPrsListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWzemPrsListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWzemPrsListData") {
				mask = mergeDpchEngData(dom);
