/**
  * \file CrdWzemPst.js
  * web client functionality for card CrdWzemPst
...
function getInitdone(pnlshort) {
	return(retrieveSi(srcdoc, "StatAppWzemPst", "initdone" + pnlshort) == "true");
};

function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppWzemPst", "initdone" + pnlshort, "true");

	checkInitdone();
};

function checkInitdone() {
	var initdoneHeadbar = (retrieveSi(srcdoc, "StatAppWzemPst", "initdoneHeadbar") == "true");
	var initdoneList = (retrieveSi(srcdoc, "StatAppWzemPst", "initdoneList") == "true");
	var initdoneRec = (retrieveSi(srcdoc, "StatAppWzemPst", "initdoneRec") == "true");

	if (!initdoneHeadbar) {
		doc.getElementById("Headbar").src = "./PnlWzemPstHeadbar.html?scrJref=" + scrJrefHeadbar;
	} else if (!initdoneList) {
		doc.getElementById("List").src = "./PnlWzemPstList.html?scrJref=" + scrJrefList;
	} else if (!initdoneRec) {
		doc.getElementById("Rec").src = "./PnlWzemPstRec.html?scrJref=" + scrJrefRec;
	} else {
		doc.initdone = true;
...
function toggleMenu(short, left, width, height) {
	var shortMenu = retrieveSi(srcdoc, "StatAppWzemPst", "shortMenu");
...
function showMenu(short, left, width, height) {
	...
	setSi(srcdoc, "StatAppWzemPst", "shortMenu", short);
	setSi(srcdoc, "StatAppWzemPst", "widthMenu", "" + (width-3));
...
function hideMenu(short) {
	...
	setSi(srcdoc, "StatAppWzemPst", "shortMenu", "");
...
function initMenApp() {
	...
	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppWzemPst", "widthMenu"));

	setTextContent(mendoc, mendoc.getElementById("MitAppAbt"), retrieveTi(srcdoc, "TagWzemPst", "MitAppAbt"));
	setTextContent(mendoc, mendoc.getElementById("MrlAppHlp"), retrieveTi(srcdoc, "TagWzemPst", "MrlAppHlp"));
};
...
function initMenCrd() {
	...
	setTextContent(mendoc, mendoc.getElementById("MtxCrdPst"), retrieveCi(srcdoc, "ContInfWzemPst", "MtxCrdPst"));
};
...
function refresh() {
	...
	doc.title = retrieveCi(srcdoc, "ContInfWzemPst", "MtxCrdPst") + " - WhizniumSBE Engine Monitor v0.9.19";
};
...
function handleUnload() {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzemPstDo", scrJref, "close");
	sendReq(str, doc, null, true);
};

function handleMitClick(menshort, consref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzemPstDo", scrJref, consref + "Click");
	sendReq(str, doc, handleDpchAppDoReply);
...
function handleMrlClick(menshort, consref) {
	window.open(retrieveCi(srcdoc, "ContInfWzemPst", consref), "_blank");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWzemPstData", "ContInfWzemPst", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWzemPstData", "FeedFSge", srcdoc)) mask.push("feedFSge");
	if (updateSrcblock(dom, "DpchEngWzemPstData", "StatAppWzemPst", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWzemPstData", "StatShrWzemPst", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWzemPstData", "TagWzemPst", srcdoc)) mask.push("tag");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWzemPstData") {
				mergeDpchEngData(dom);

				// establish sub-panel scrJref's as global variables, and start sub-panel load sequence
				scrJrefHeadbar = retrieveSi(srcdoc, "StatShrWzemPst", "scrJrefHeadbar");
				scrJrefList = retrieveSi(srcdoc, "StatShrWzemPst", "scrJrefList");
				scrJrefRec = retrieveSi(srcdoc, "StatShrWzemPst", "scrJrefRec");
		...
		if (blk) {
			...
			if (_scrJref != "") {
				if (_scrJref == scrJref) {
					...
					} else if (blk.nodeName == "DpchEngWzemPstData") {
						mergeDpchEngData(dom);
						refresh();
					};
...
function restartNotify() {
	var srefIxWzemVReqitmode = retrieveSi(srcdoc, "StatAppWzemPst", "srefIxWzemVReqitmode");

	if (srefIxWzemVReqitmode != "notify") {
		if (doc.timeout) window.clearTimeout(doc.timeout);

		srefIxWzemVReqitmode = "notify";
		setSi(srcdoc, "StatAppWzemPst", "srefIxWzemVReqitmode", srefIxWzemVReqitmode);
...
function iterateReqit(ev) {
	...
	var srefIxWzemVReqitmode = retrieveSi(srcdoc, "StatAppWzemPst", "srefIxWzemVReqitmode");
	var latency = parseInt(retrieveSi(srcdoc, "StatAppWzemPst", "latency"));
