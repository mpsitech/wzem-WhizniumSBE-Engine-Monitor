...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlWzemPstList_aside.html";
	doc.getElementById("List_cont").src = "./PnlWzemPstList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlWzemPstList_bside.html";
	doc.getElementById("List_cont").src = "./PnlWzemPstList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzemPstList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWzemPstList", "Trs"));
...
function init() {
	var srefIxWzemVExpstate = retrieveSi(srcdoc, "StatShrWzemPstList", "srefIxWzemVExpstate");
...
function refreshA() {
	var TxtFor = retrieveCi(srcdoc, "ContInfWzemPstList", "TxtFor");

	setTextContent(contdoc, contdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWzemPstList", "TxtFor"), TxtFor));

	if (TxtFor == "") {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzemPstList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWzemPstList", "ntot") + ")");
		clearElem(contdoc.getElementById("TxtPre"));
	} else {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzemPstList", "Cpt"));
		setTextContent(contdoc, contdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWzemPstList", "TxtPre") + " (" + retrieveSi(srcdoc, "StatShrQryWzemPstList", "ntot") + ")");
	};
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWzemPstList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemPstList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemPstList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemPstList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemPstList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWzemPstList", "ntot"));
	...
	// header row
	setTextContent(hdrdoc, hdrdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWzemPstList", "TxtFor"), TxtFor));

	if (TxtFor == "") clearElem(hdrdoc.getElementById("TxtPre"));
	else setTextContent(hdrdoc, hdrdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWzemPstList", "TxtPre"));

	// update column characteristics
	if (chgcol) {
		// obtain column (header) characteristics
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemPstList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemPstList", "ncol"));

		var TcoJobWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemPstList", "TcoJobWidth"));
		var TcoPstWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemPstList", "TcoPstWidth"));
		var TcoStaWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemPstList", "TcoStaWidth"));
		var TcoStoWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemPstList", "TcoStoWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacWzemPstList", "numFTos"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoPst"); doc.widths.push(TcoPstWidth); doc.fs.push("pst"); doc.tos.push(0); wtot += TcoPstWidth;
			} else if (i == 3) {
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagWzemPstList", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWzemPstList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWzemQPstList
		rownode = getRownode(srcdoc, "ListWzemQPstList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWzemPstList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWzemPstList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWzemPstList", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WzemPstList", "ListWzemQPstList", retrieveTi(srcdoc, "TagWzemPstList", "TxtRecord1"), retrieveTi(srcdoc, "TagWzemPstList", "TxtRecord2"),
				["Job","Pst","Sta","Sto"],
				["job","pst","sta","sto"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzemPstList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemPstList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemPstList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWzemPstList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWzemPstList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWzemPstListData", scrJref, "StgIacQryWzemPstList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemPstList", "firstcol"));

	setSi(srcdoc, "StatAppQryWzemPstList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzemPstList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzemPstListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacWzemPstList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacWzemPstList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppWzemPstListData", scrJref, "ContIacWzemPstList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWzemPstListData", "ContIacWzemPstList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngWzemPstListData", "ContInfWzemPstList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWzemPstListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWzemPstListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngWzemPstListData", "StatShrWzemPstList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWzemPstListData", "StgIacWzemPstList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWzemPstListData", "TagWzemPstList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWzemPstListData", "ListWzemQPstList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWzemPstListData", "StatAppQryWzemPstList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWzemPstListData", "StatShrQryWzemPstList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWzemPstListData", "StgIacQryWzemPstList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngWzemPstListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWzemPstListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWzemPstListData") {
				mask = mergeDpchEngData(dom);
