...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlWzemUsrList_aside.html";
	doc.getElementById("List_cont").src = "./PnlWzemUsrList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlWzemUsrList_bside.html";
	doc.getElementById("List_cont").src = "./PnlWzemUsrList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzemUsrList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWzemUsrList", "Trs"));
...
function init() {
	var srefIxWzemVExpstate = retrieveSi(srcdoc, "StatShrWzemUsrList", "srefIxWzemVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzemUsrList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWzemUsrList", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWzemUsrList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemUsrList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemUsrList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemUsrList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemUsrList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWzemUsrList", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemUsrList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemUsrList", "ncol"));

		var TcoGrpWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemUsrList", "TcoGrpWidth"));
		var TcoOwnWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemUsrList", "TcoOwnWidth"));
		var TcoPrsWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemUsrList", "TcoPrsWidth"));
		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemUsrList", "TcoSrfWidth"));
		var TcoUsgWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemUsrList", "TcoUsgWidth"));
		var TcoSteWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemUsrList", "TcoSteWidth"));
		var TcoLclWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemUsrList", "TcoLclWidth"));
		var TcoUlvWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemUsrList", "TcoUlvWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacWzemUsrList", "numFTos"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoPrs"); doc.widths.push(TcoPrsWidth); doc.fs.push("prs"); doc.tos.push(6); wtot += TcoPrsWidth;
			} else if (i == 4) {
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagWzemUsrList", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWzemUsrList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWzemQUsrList
		rownode = getRownode(srcdoc, "ListWzemQUsrList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWzemUsrList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWzemUsrList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWzemUsrList", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WzemUsrList", "ListWzemQUsrList", retrieveTi(srcdoc, "TagWzemUsrList", "TxtRecord1"), retrieveTi(srcdoc, "TagWzemUsrList", "TxtRecord2"),
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzemUsrList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemUsrList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemUsrList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWzemUsrList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWzemUsrList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWzemUsrListData", scrJref, "StgIacQryWzemUsrList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemUsrList", "firstcol"));

	setSi(srcdoc, "StatAppQryWzemUsrList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzemUsrList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzemUsrListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacWzemUsrList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacWzemUsrList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppWzemUsrListData", scrJref, "ContIacWzemUsrList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWzemUsrListData", "ContIacWzemUsrList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngWzemUsrListData", "ContInfWzemUsrList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWzemUsrListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWzemUsrListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngWzemUsrListData", "StatShrWzemUsrList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWzemUsrListData", "StgIacWzemUsrList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWzemUsrListData", "TagWzemUsrList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWzemUsrListData", "ListWzemQUsrList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWzemUsrListData", "StatAppQryWzemUsrList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWzemUsrListData", "StatShrQryWzemUsrList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWzemUsrListData", "StgIacQryWzemUsrList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngWzemUsrListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWzemUsrListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWzemUsrListData") {
				mask = mergeDpchEngData(dom);
