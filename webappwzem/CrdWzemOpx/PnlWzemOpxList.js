/**
  * \file PnlWzemOpxList.js
  * web client functionality for panel PnlWzemOpxList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlWzemOpxList_aside.html";
	doc.getElementById("List_cont").src = "./PnlWzemOpxList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlWzemOpxList_bside.html";
	doc.getElementById("List_cont").src = "./PnlWzemOpxList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzemOpxList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWzemOpxList", "Trs"));
...
function init() {
	var srefIxWzemVExpstate = retrieveSi(srcdoc, "StatShrWzemOpxList", "srefIxWzemVExpstate");
...
function refreshB(chgcol) {
	...
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWzemOpxList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemOpxList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemOpxList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemOpxList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemOpxList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWzemOpxList", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemOpxList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemOpxList", "ncol"));

		var TcoJobWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemOpxList", "TcoJobWidth"));
		var TcoXorWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemOpxList", "TcoXorWidth"));
		var TcoStaWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemOpxList", "TcoStaWidth"));
		var TcoStoWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemOpxList", "TcoStoWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacWzemOpxList", "numFTos"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagWzemOpxList", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWzemOpxList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWzemQOpxList
		rownode = getRownode(srcdoc, "ListWzemQOpxList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWzemOpxList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWzemOpxList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWzemOpxList", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WzemOpxList", "ListWzemQOpxList", retrieveTi(srcdoc, "TagWzemOpxList", "TxtRecord1"), retrieveTi(srcdoc, "TagWzemOpxList", "TxtRecord2"),
				["Job","Xor","Sta","Sto"],
				["job","xor","sta","sto"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzemOpxList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemOpxList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemOpxList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWzemOpxList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWzemOpxList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWzemOpxListData", scrJref, "StgIacQryWzemOpxList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemOpxList", "firstcol"));

	setSi(srcdoc, "StatAppQryWzemOpxList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzemOpxList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzemOpxListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacWzemOpxList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacWzemOpxList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppWzemOpxListData", scrJref, "ContIacWzemOpxList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWzemOpxListData", "ContIacWzemOpxList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngWzemOpxListData", "ContInfWzemOpxList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWzemOpxListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWzemOpxListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngWzemOpxListData", "StatShrWzemOpxList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWzemOpxListData", "StgIacWzemOpxList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWzemOpxListData", "TagWzemOpxList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWzemOpxListData", "ListWzemQOpxList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWzemOpxListData", "StatAppQryWzemOpxList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWzemOpxListData", "StatShrQryWzemOpxList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWzemOpxListData", "StgIacQryWzemOpxList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngWzemOpxListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWzemOpxListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWzemOpxListData") {
				mask = mergeDpchEngData(dom);
