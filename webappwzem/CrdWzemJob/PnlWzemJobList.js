...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlWzemJobList_aside.html";
	doc.getElementById("List_cont").src = "./PnlWzemJobList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlWzemJobList_bside.html";
	doc.getElementById("List_cont").src = "./PnlWzemJobList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzemJobList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWzemJobList", "Trs"));
...
function init() {
	var srefIxWzemVExpstate = retrieveSi(srcdoc, "StatShrWzemJobList", "srefIxWzemVExpstate");
...
function refreshB(chgcol) {
	...
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWzemJobList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemJobList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemJobList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemJobList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemJobList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWzemJobList", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemJobList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemJobList", "ncol"));

		var TcoPrdWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemJobList", "TcoPrdWidth"));
		var TcoJobWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemJobList", "TcoJobWidth"));
		var TcoXjrWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemJobList", "TcoXjrWidth"));
		var TcoStaWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemJobList", "TcoStaWidth"));
		var TcoStoWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemJobList", "TcoStoWidth"));
		var TcoSupWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemJobList", "TcoSupWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacWzemJobList", "numFTos"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				doc.cols.push("TcoPrd"); doc.widths.push(TcoPrdWidth); doc.fs.push("prd"); doc.tos.push(4); wtot += TcoPrdWidth;
			} else if (i == 2) {
				doc.cols.push("TcoJob"); doc.widths.push(TcoJobWidth); doc.fs.push("job"); doc.tos.push(0); wtot += TcoJobWidth;
			} else if (i == 3) {
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagWzemJobList", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWzemJobList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWzemQJobList
		rownode = getRownode(srcdoc, "ListWzemQJobList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWzemJobList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWzemJobList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWzemJobList", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WzemJobList", "ListWzemQJobList", retrieveTi(srcdoc, "TagWzemJobList", "TxtRecord1"), retrieveTi(srcdoc, "TagWzemJobList", "TxtRecord2"),
				["Prd","Job","Xjr","Sta","Sto","Sup"],
				["prd","job","xjr","sta","sto","sup"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzemJobList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemJobList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemJobList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWzemJobList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWzemJobList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWzemJobListData", scrJref, "StgIacQryWzemJobList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemJobList", "firstcol"));

	setSi(srcdoc, "StatAppQryWzemJobList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzemJobList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzemJobListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacWzemJobList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacWzemJobList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppWzemJobListData", scrJref, "ContIacWzemJobList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWzemJobListData", "ContIacWzemJobList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngWzemJobListData", "ContInfWzemJobList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWzemJobListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWzemJobListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngWzemJobListData", "StatShrWzemJobList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWzemJobListData", "StgIacWzemJobList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWzemJobListData", "TagWzemJobList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWzemJobListData", "ListWzemQJobList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWzemJobListData", "StatAppQryWzemJobList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWzemJobListData", "StatShrQryWzemJobList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWzemJobListData", "StgIacQryWzemJobList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngWzemJobListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWzemJobListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWzemJobListData") {
				mask = mergeDpchEngData(dom);
