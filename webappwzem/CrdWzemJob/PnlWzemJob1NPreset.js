...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWzemJob1NPreset", "srefIxWzemVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWzemJob1NPreset", "srefIxWzemVExpstate", "mind");
	...
	// change content
	doc.getElementById("1NPreset_side").src = "./PnlWzemJob1NPreset_aside.html";
	doc.getElementById("1NPreset_cont").src = "./PnlWzemJob1NPreset_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWzemJob1NPreset", "srefIxWzemVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWzemJob1NPreset", "srefIxWzemVExpstate", "regd");
	...
	// change content
	doc.getElementById("1NPreset_side").src = "./PnlWzemJob1NPreset_bside.html";
	doc.getElementById("1NPreset_cont").src = "./PnlWzemJob1NPreset_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzemJob1NPreset", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWzemJob1NPreset", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWzemVExpstate = retrieveSi(srcdoc, "StatAppWzemJob1NPreset", "srefIxWzemVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzemJob1NPreset", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWzemJob1NPreset", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWzemJob1NPreset", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemJob1NPreset", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemJob1NPreset", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemJob1NPreset", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemJob1NPreset", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWzemJob1NPreset", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemJob1NPreset", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemJob1NPreset", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemJob1NPreset", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWzemJob1NPreset", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWzemJob1NPreset", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWzemQJob1NPreset
		rownode = getRownode(srcdoc, "ListWzemQJob1NPreset", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWzemJob1NPreset", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWzemJob1NPreset", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWzemJob1NPreset", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzemJob1NPreset", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemJob1NPreset", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemJob1NPreset", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWzemJob1NPreset", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWzemJob1NPreset", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWzemJob1NPresetData", scrJref, "StgIacQryWzemJob1NPreset");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWzemJob1NPreset", "srefIxWzemVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemJob1NPreset", "firstcol"));

	setSi(srcdoc, "StatAppQryWzemJob1NPreset", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWzemJob1NPreset", "srefIxWzemVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzemJob1NPreset", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzemJob1NPresetDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWzemJob1NPresetData", "ContInfWzemJob1NPreset", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWzemJob1NPresetData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWzemJob1NPresetData", "StatAppWzemJob1NPreset", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWzemJob1NPresetData", "StatShrWzemJob1NPreset", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWzemJob1NPresetData", "StgIacWzemJob1NPreset", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWzemJob1NPresetData", "TagWzemJob1NPreset", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWzemJob1NPresetData", "ListWzemQJob1NPreset", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWzemJob1NPresetData", "StatAppQryWzemJob1NPreset", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWzemJob1NPresetData", "StatShrQryWzemJob1NPreset", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWzemJob1NPresetData", "StgIacQryWzemJob1NPreset", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWzemJob1NPresetData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWzemJob1NPresetData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWzemJob1NPresetData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
