/**
  * \file PnlWzemJob1NClstn.js
  * web client functionality for panel PnlWzemJob1NClstn
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWzemJob1NClstn", "srefIxWzemVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWzemJob1NClstn", "srefIxWzemVExpstate", "mind");

	getRecwnd().changeHeight("1NClstn", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("1NClstn_side").setAttribute("height", "30");
	doc.getElementById("1NClstn_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("1NClstn_side").src = "./PnlWzemJob1NClstn_aside.html";
	doc.getElementById("1NClstn_cont").src = "./PnlWzemJob1NClstn_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWzemJob1NClstn", "srefIxWzemVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWzemJob1NClstn", "srefIxWzemVExpstate", "regd");

	getRecwnd().changeHeight("1NClstn", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("1NClstn_side").setAttribute("height", "255");
	doc.getElementById("1NClstn_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("1NClstn_side").src = "./PnlWzemJob1NClstn_bside.html";
	doc.getElementById("1NClstn_cont").src = "./PnlWzemJob1NClstn_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("1NClstn_side").contentDocument;
	contdoc = doc.getElementById("1NClstn_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("1NClstn_side").contentDocument;
	contdoc = doc.getElementById("1NClstn_cont").contentDocument;
	hdrdoc = contdoc.getElementById("1NClstn_hdr").contentDocument;
	tbldoc = contdoc.getElementById("1NClstn_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("1NClstn_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzemJob1NClstn", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWzemJob1NClstn", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWzemVExpstate = retrieveSi(srcdoc, "StatAppWzemJob1NClstn", "srefIxWzemVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzemJob1NClstn", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWzemJob1NClstn", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWzemJob1NClstn", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemJob1NClstn", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemJob1NClstn", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemJob1NClstn", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemJob1NClstn", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWzemJob1NClstn", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemJob1NClstn", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemJob1NClstn", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemJob1NClstn", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWzemJob1NClstn", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWzemJob1NClstn", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWzemQJob1NClstn
		rownode = getRownode(srcdoc, "ListWzemQJob1NClstn", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWzemJob1NClstn", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWzemJob1NClstn", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWzemJob1NClstn", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("1NClstn", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("1NClstn_side").setAttribute("height", "" + (height+55));
	doc.getElementById("1NClstn_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("1NClstn_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("1NClstn_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WzemJob1NClstn", "ListWzemQJob1NClstn", retrieveTi(srcdoc, "TagWzemJob1NClstn", "TxtRecord1"), retrieveTi(srcdoc, "TagWzemJob1NClstn", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzemJob1NClstn", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemJob1NClstn", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemJob1NClstn", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWzemJob1NClstn", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWzemJob1NClstn", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWzemJob1NClstnData", scrJref, "StgIacQryWzemJob1NClstn");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWzemJob1NClstn", "srefIxWzemVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemJob1NClstn", "firstcol"));

	setSi(srcdoc, "StatAppQryWzemJob1NClstn", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWzemJob1NClstn", "srefIxWzemVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzemJob1NClstn", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzemJob1NClstnDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWzemJob1NClstnData", "ContInfWzemJob1NClstn", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWzemJob1NClstnData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWzemJob1NClstnData", "StatAppWzemJob1NClstn", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWzemJob1NClstnData", "StatShrWzemJob1NClstn", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWzemJob1NClstnData", "StgIacWzemJob1NClstn", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWzemJob1NClstnData", "TagWzemJob1NClstn", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWzemJob1NClstnData", "ListWzemQJob1NClstn", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWzemJob1NClstnData", "StatAppQryWzemJob1NClstn", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWzemJob1NClstnData", "StatShrQryWzemJob1NClstn", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWzemJob1NClstnData", "StgIacQryWzemJob1NClstn", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWzemJob1NClstnData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWzemJob1NClstnData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("1NClstn");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWzemJob1NClstnData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
