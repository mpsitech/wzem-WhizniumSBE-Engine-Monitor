/**
  * \file PnlWzemJob1NOp.js
  * web client functionality for panel PnlWzemJob1NOp
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWzemJob1NOp", "srefIxWzemVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWzemJob1NOp", "srefIxWzemVExpstate", "mind");
	...
	// change content
	doc.getElementById("1NOp_side").src = "./PnlWzemJob1NOp_aside.html";
	doc.getElementById("1NOp_cont").src = "./PnlWzemJob1NOp_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWzemJob1NOp", "srefIxWzemVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWzemJob1NOp", "srefIxWzemVExpstate", "regd");
	...
	// change content
	doc.getElementById("1NOp_side").src = "./PnlWzemJob1NOp_bside.html";
	doc.getElementById("1NOp_cont").src = "./PnlWzemJob1NOp_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzemJob1NOp", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWzemJob1NOp", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWzemVExpstate = retrieveSi(srcdoc, "StatAppWzemJob1NOp", "srefIxWzemVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzemJob1NOp", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWzemJob1NOp", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWzemJob1NOp", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemJob1NOp", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemJob1NOp", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemJob1NOp", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemJob1NOp", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWzemJob1NOp", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemJob1NOp", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemJob1NOp", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemJob1NOp", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWzemJob1NOp", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWzemJob1NOp", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWzemQJob1NOp
		rownode = getRownode(srcdoc, "ListWzemQJob1NOp", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWzemJob1NOp", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWzemJob1NOp", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWzemJob1NOp", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WzemJob1NOp", "ListWzemQJob1NOp", retrieveTi(srcdoc, "TagWzemJob1NOp", "TxtRecord1"), retrieveTi(srcdoc, "TagWzemJob1NOp", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzemJob1NOp", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemJob1NOp", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemJob1NOp", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWzemJob1NOp", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWzemJob1NOp", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWzemJob1NOpData", scrJref, "StgIacQryWzemJob1NOp");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWzemJob1NOp", "srefIxWzemVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemJob1NOp", "firstcol"));

	setSi(srcdoc, "StatAppQryWzemJob1NOp", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWzemJob1NOp", "srefIxWzemVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzemJob1NOp", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzemJob1NOpDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWzemJob1NOpData", "ContInfWzemJob1NOp", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWzemJob1NOpData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWzemJob1NOpData", "StatAppWzemJob1NOp", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWzemJob1NOpData", "StatShrWzemJob1NOp", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWzemJob1NOpData", "StgIacWzemJob1NOp", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWzemJob1NOpData", "TagWzemJob1NOp", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWzemJob1NOpData", "ListWzemQJob1NOp", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWzemJob1NOpData", "StatAppQryWzemJob1NOp", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWzemJob1NOpData", "StatShrQryWzemJob1NOp", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWzemJob1NOpData", "StgIacQryWzemJob1NOp", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWzemJob1NOpData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWzemJob1NOpData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWzemJob1NOpData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
