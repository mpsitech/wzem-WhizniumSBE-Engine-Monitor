/**
  * \file CrdWzemJob.js
  * web client functionality for card CrdWzemJob
...
function getInitdone(pnlshort) {
	return(retrieveSi(srcdoc, "StatAppWzemJob", "initdone" + pnlshort) == "true");
};

function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppWzemJob", "initdone" + pnlshort, "true");

	checkInitdone();
};

function checkInitdone() {
	var initdoneHeadbar = (retrieveSi(srcdoc, "StatAppWzemJob", "initdoneHeadbar") == "true");
	var initdoneList = (retrieveSi(srcdoc, "StatAppWzemJob", "initdoneList") == "true");
	var initdoneRec = (retrieveSi(srcdoc, "StatAppWzemJob", "initdoneRec") == "true");

	if (!initdoneHeadbar) {
		doc.getElementById("Headbar").src = "./PnlWzemJobHeadbar.html?scrJref=" + scrJrefHeadbar;
	} else if (!initdoneList) {
		doc.getElementById("List").src = "./PnlWzemJobList.html?scrJref=" + scrJrefList;
	} else if (!initdoneRec) {
		doc.getElementById("Rec").src = "./PnlWzemJobRec.html?scrJref=" + scrJrefRec;
	} else {
		doc.initdone = true;
...
function toggleMenu(short, left, width, height) {
	var shortMenu = retrieveSi(srcdoc, "StatAppWzemJob", "shortMenu");
...
function showMenu(short, left, width, height) {
	...
	setSi(srcdoc, "StatAppWzemJob", "shortMenu", short);
	setSi(srcdoc, "StatAppWzemJob", "widthMenu", "" + (width-3));
...
function hideMenu(short) {
	...
	setSi(srcdoc, "StatAppWzemJob", "shortMenu", "");
...
function initMenApp() {
	...
	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppWzemJob", "widthMenu"));

	setTextContent(mendoc, mendoc.getElementById("MitAppAbt"), retrieveTi(srcdoc, "TagWzemJob", "MitAppAbt"));
	setTextContent(mendoc, mendoc.getElementById("MrlAppHlp"), retrieveTi(srcdoc, "TagWzemJob", "MrlAppHlp"));
};
...
function initMenCrd() {
	...
	setTextContent(mendoc, mendoc.getElementById("MtxCrdJob"), retrieveCi(srcdoc, "ContInfWzemJob", "MtxCrdJob"));
};
...
function refresh() {
	...
	doc.title = retrieveCi(srcdoc, "ContInfWzemJob", "MtxCrdJob") + " - WhizniumSBE Engine Monitor v0.9.19";
};
...
function handleUnload() {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzemJobDo", scrJref, "close");
	sendReq(str, doc, null, true);
};

function handleMitClick(menshort, consref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzemJobDo", scrJref, consref + "Click");
	sendReq(str, doc, handleDpchAppDoReply);
...
function handleMrlClick(menshort, consref) {
	window.open(retrieveCi(srcdoc, "ContInfWzemJob", consref), "_blank");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWzemJobData", "ContInfWzemJob", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWzemJobData", "FeedFSge", srcdoc)) mask.push("feedFSge");
	if (updateSrcblock(dom, "DpchEngWzemJobData", "StatAppWzemJob", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWzemJobData", "StatShrWzemJob", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWzemJobData", "TagWzemJob", srcdoc)) mask.push("tag");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWzemJobData") {
				mergeDpchEngData(dom);

				// establish sub-panel scrJref's as global variables, and start sub-panel load sequence
				scrJrefHeadbar = retrieveSi(srcdoc, "StatShrWzemJob", "scrJrefHeadbar");
				scrJrefList = retrieveSi(srcdoc, "StatShrWzemJob", "scrJrefList");
				scrJrefRec = retrieveSi(srcdoc, "StatShrWzemJob", "scrJrefRec");
		...
		if (blk) {
			...
			if (_scrJref != "") {
				if (_scrJref == scrJref) {
					...
					} else if (blk.nodeName == "DpchEngWzemJobData") {
						mergeDpchEngData(dom);
						refresh();
					};
...
function restartNotify() {
	var srefIxWzemVReqitmode = retrieveSi(srcdoc, "StatAppWzemJob", "srefIxWzemVReqitmode");

	if (srefIxWzemVReqitmode != "notify") {
		if (doc.timeout) window.clearTimeout(doc.timeout);

		srefIxWzemVReqitmode = "notify";
		setSi(srcdoc, "StatAppWzemJob", "srefIxWzemVReqitmode", srefIxWzemVReqitmode);
...
function iterateReqit(ev) {
	...
	var srefIxWzemVReqitmode = retrieveSi(srcdoc, "StatAppWzemJob", "srefIxWzemVReqitmode");
	var latency = parseInt(retrieveSi(srcdoc, "StatAppWzemJob", "latency"));
