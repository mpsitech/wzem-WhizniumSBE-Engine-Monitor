...
function minimize(updh) {
	...
	setSi(srcdoc, "StatAppWzemJobSup1NJob", "srefIxWzemVExpstate", "mind");

	getRecwnd().changeHeight("Sup1NJob", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("Sup1NJob_side").setAttribute("height", "30");
	doc.getElementById("Sup1NJob_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("Sup1NJob_side").src = "./PnlWzemJobSup1NJob_aside.html";
	doc.getElementById("Sup1NJob_cont").src = "./PnlWzemJobSup1NJob_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWzemJobSup1NJob", "srefIxWzemVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWzemJobSup1NJob", "srefIxWzemVExpstate", "regd");

	getRecwnd().changeHeight("Sup1NJob", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("Sup1NJob_side").setAttribute("height", "255");
	doc.getElementById("Sup1NJob_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("Sup1NJob_side").src = "./PnlWzemJobSup1NJob_bside.html";
	doc.getElementById("Sup1NJob_cont").src = "./PnlWzemJobSup1NJob_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Sup1NJob_side").contentDocument;
	contdoc = doc.getElementById("Sup1NJob_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("Sup1NJob_side").contentDocument;
	contdoc = doc.getElementById("Sup1NJob_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Sup1NJob_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Sup1NJob_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Sup1NJob_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzemJobSup1NJob", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWzemJobSup1NJob", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWzemVExpstate = retrieveSi(srcdoc, "StatAppWzemJobSup1NJob", "srefIxWzemVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzemJobSup1NJob", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWzemJobSup1NJob", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWzemJobSup1NJob", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemJobSup1NJob", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemJobSup1NJob", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemJobSup1NJob", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemJobSup1NJob", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWzemJobSup1NJob", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemJobSup1NJob", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemJobSup1NJob", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemJobSup1NJob", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWzemJobSup1NJob", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWzemJobSup1NJob", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWzemQJobSup1NJob
		rownode = getRownode(srcdoc, "ListWzemQJobSup1NJob", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWzemJobSup1NJob", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWzemJobSup1NJob", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWzemJobSup1NJob", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Sup1NJob", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Sup1NJob_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Sup1NJob_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Sup1NJob_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Sup1NJob_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WzemJobSup1NJob", "ListWzemQJobSup1NJob", retrieveTi(srcdoc, "TagWzemJobSup1NJob", "TxtRecord1"), retrieveTi(srcdoc, "TagWzemJobSup1NJob", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzemJobSup1NJob", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemJobSup1NJob", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemJobSup1NJob", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWzemJobSup1NJob", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWzemJobSup1NJob", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWzemJobSup1NJobData", scrJref, "StgIacQryWzemJobSup1NJob");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWzemJobSup1NJob", "srefIxWzemVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemJobSup1NJob", "firstcol"));

	setSi(srcdoc, "StatAppQryWzemJobSup1NJob", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWzemJobSup1NJob", "srefIxWzemVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzemJobSup1NJob", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzemJobSup1NJobDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWzemJobSup1NJobData", "ContInfWzemJobSup1NJob", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWzemJobSup1NJobData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWzemJobSup1NJobData", "StatAppWzemJobSup1NJob", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWzemJobSup1NJobData", "StatShrWzemJobSup1NJob", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWzemJobSup1NJobData", "StgIacWzemJobSup1NJob", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWzemJobSup1NJobData", "TagWzemJobSup1NJob", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWzemJobSup1NJobData", "ListWzemQJobSup1NJob", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWzemJobSup1NJobData", "StatAppQryWzemJobSup1NJob", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWzemJobSup1NJobData", "StatShrQryWzemJobSup1NJob", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWzemJobSup1NJobData", "StgIacQryWzemJobSup1NJob", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWzemJobSup1NJobData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWzemJobSup1NJobData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Sup1NJob");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWzemJobSup1NJobData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
