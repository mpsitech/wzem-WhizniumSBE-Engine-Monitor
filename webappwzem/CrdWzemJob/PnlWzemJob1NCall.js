...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWzemJob1NCall", "srefIxWzemVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWzemJob1NCall", "srefIxWzemVExpstate", "mind");
	...
	// change content
	doc.getElementById("1NCall_side").src = "./PnlWzemJob1NCall_aside.html";
	doc.getElementById("1NCall_cont").src = "./PnlWzemJob1NCall_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWzemJob1NCall", "srefIxWzemVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWzemJob1NCall", "srefIxWzemVExpstate", "regd");
	...
	// change content
	doc.getElementById("1NCall_side").src = "./PnlWzemJob1NCall_bside.html";
	doc.getElementById("1NCall_cont").src = "./PnlWzemJob1NCall_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzemJob1NCall", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWzemJob1NCall", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWzemVExpstate = retrieveSi(srcdoc, "StatAppWzemJob1NCall", "srefIxWzemVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzemJob1NCall", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWzemJob1NCall", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWzemJob1NCall", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemJob1NCall", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemJob1NCall", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemJob1NCall", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemJob1NCall", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWzemJob1NCall", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemJob1NCall", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemJob1NCall", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemJob1NCall", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWzemJob1NCall", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWzemJob1NCall", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWzemQJob1NCall
		rownode = getRownode(srcdoc, "ListWzemQJob1NCall", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWzemJob1NCall", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWzemJob1NCall", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWzemJob1NCall", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzemJob1NCall", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemJob1NCall", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemJob1NCall", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWzemJob1NCall", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWzemJob1NCall", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWzemJob1NCallData", scrJref, "StgIacQryWzemJob1NCall");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWzemJob1NCall", "srefIxWzemVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemJob1NCall", "firstcol"));

	setSi(srcdoc, "StatAppQryWzemJob1NCall", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWzemJob1NCall", "srefIxWzemVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzemJob1NCall", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzemJob1NCallDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWzemJob1NCallData", "ContInfWzemJob1NCall", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWzemJob1NCallData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWzemJob1NCallData", "StatAppWzemJob1NCall", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWzemJob1NCallData", "StatShrWzemJob1NCall", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWzemJob1NCallData", "StgIacWzemJob1NCall", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWzemJob1NCallData", "TagWzemJob1NCall", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWzemJob1NCallData", "ListWzemQJob1NCall", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWzemJob1NCallData", "StatAppQryWzemJob1NCall", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWzemJob1NCallData", "StatShrQryWzemJob1NCall", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWzemJob1NCallData", "StgIacQryWzemJob1NCall", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWzemJob1NCallData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWzemJob1NCallData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWzemJob1NCallData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
