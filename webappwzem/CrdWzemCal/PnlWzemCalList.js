...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlWzemCalList_aside.html";
	doc.getElementById("List_cont").src = "./PnlWzemCalList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlWzemCalList_bside.html";
	doc.getElementById("List_cont").src = "./PnlWzemCalList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzemCalList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWzemCalList", "Trs"));
...
function init() {
	var srefIxWzemVExpstate = retrieveSi(srcdoc, "StatShrWzemCalList", "srefIxWzemVExpstate");
...
function refreshA() {
	var TxtFor = retrieveCi(srcdoc, "ContInfWzemCalList", "TxtFor");

	setTextContent(contdoc, contdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWzemCalList", "TxtFor"), TxtFor));

	if (TxtFor == "") {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzemCalList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWzemCalList", "ntot") + ")");
		clearElem(contdoc.getElementById("TxtPre"));
	} else {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWzemCalList", "Cpt"));
		setTextContent(contdoc, contdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWzemCalList", "TxtPre") + " (" + retrieveSi(srcdoc, "StatShrQryWzemCalList", "ntot") + ")");
	};
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWzemCalList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemCalList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemCalList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemCalList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemCalList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWzemCalList", "ntot"));
	...
	// header row
	setTextContent(hdrdoc, hdrdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWzemCalList", "TxtFor"), TxtFor));

	if (TxtFor == "") clearElem(hdrdoc.getElementById("TxtPre"));
	else setTextContent(hdrdoc, hdrdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWzemCalList", "TxtPre"));

	// update column characteristics
	if (chgcol) {
		// obtain column (header) characteristics
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemCalList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemCalList", "ncol"));

		var TcoJobWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemCalList", "TcoJobWidth"));
		var TcoCalWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemCalList", "TcoCalWidth"));
		var TcoStaWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemCalList", "TcoStaWidth"));
		var TcoStoWidth = parseInt(retrieveSi(srcdoc, "StgIacWzemCalList", "TcoStoWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacWzemCalList", "numFTos"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagWzemCalList", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWzemCalList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWzemQCalList
		rownode = getRownode(srcdoc, "ListWzemQCalList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWzemCalList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWzemCalList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWzemCalList", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzemCalList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWzemCalList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWzemCalList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWzemCalList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWzemCalList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWzemCalListData", scrJref, "StgIacQryWzemCalList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWzemCalList", "firstcol"));

	setSi(srcdoc, "StatAppQryWzemCalList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWzemCalList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzemCalListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacWzemCalList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacWzemCalList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppWzemCalListData", scrJref, "ContIacWzemCalList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWzemCalListData", "ContIacWzemCalList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngWzemCalListData", "ContInfWzemCalList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWzemCalListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWzemCalListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngWzemCalListData", "StatShrWzemCalList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWzemCalListData", "StgIacWzemCalList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWzemCalListData", "TagWzemCalList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWzemCalListData", "ListWzemQCalList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWzemCalListData", "StatAppQryWzemCalList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWzemCalListData", "StatShrQryWzemCalList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWzemCalListData", "StgIacQryWzemCalList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngWzemCalListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWzemCalListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWzemCalListData") {
				mask = mergeDpchEngData(dom);
