...
function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppWzemCal", "initdone" + pnlshort, "true");

	checkInitdone();
};

function checkInitdone() {
	var initdoneHeadbar = (retrieveSi(srcdoc, "StatAppWzemCal", "initdoneHeadbar") == "true");
	var initdoneList = (retrieveSi(srcdoc, "StatAppWzemCal", "initdoneList") == "true");
	var initdoneRec = (retrieveSi(srcdoc, "StatAppWzemCal", "initdoneRec") == "true");

	if (!initdoneHeadbar) {
		doc.getElementById("Headbar").src = "./PnlWzemCalHeadbar.html?scrJref=" + scrJrefHeadbar;
	} else if (!initdoneList) {
		doc.getElementById("List").src = "./PnlWzemCalList.html?scrJref=" + scrJrefList;
	} else if (!initdoneRec) {
		doc.getElementById("Rec").src = "./PnlWzemCalRec.html?scrJref=" + scrJrefRec;
	} else {
		doc.initdone = true;
...
function toggleMenu(short, left, width, height) {
	var shortMenu = retrieveSi(srcdoc, "StatAppWzemCal", "shortMenu");
...
function showMenu(short, left, width, height) {
	...
	setSi(srcdoc, "StatAppWzemCal", "shortMenu", short);
	setSi(srcdoc, "StatAppWzemCal", "widthMenu", "" + (width-3));
...
function hideMenu(short) {
	...
	setSi(srcdoc, "StatAppWzemCal", "shortMenu", "");
...
function initMenApp() {
	...
	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppWzemCal", "widthMenu"));

	setTextContent(mendoc, mendoc.getElementById("MitAppAbt"), retrieveTi(srcdoc, "TagWzemCal", "MitAppAbt"));
	setTextContent(mendoc, mendoc.getElementById("MrlAppHlp"), retrieveTi(srcdoc, "TagWzemCal", "MrlAppHlp"));
};
...
function initMenCrd() {
	...
	setTextContent(mendoc, mendoc.getElementById("MtxCrdCal"), retrieveCi(srcdoc, "ContInfWzemCal", "MtxCrdCal"));
};
...
function refresh() {
	...
	doc.title = retrieveCi(srcdoc, "ContInfWzemCal", "MtxCrdCal") + " - WhizniumSBE Engine Monitor v1.0.7";
};
...
function handleUnload() {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzemCalDo", scrJref, "close");
	sendReq(str, doc, null, true);
};

function handleMitClick(menshort, consref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWzemCalDo", scrJref, consref + "Click");
	sendReq(str, doc, handleDpchAppDoReply);
...
function handleMrlClick(menshort, consref) {
	window.open(retrieveCi(srcdoc, "ContInfWzemCal", consref), "_blank");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWzemCalData", "ContInfWzemCal", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWzemCalData", "FeedFSge", srcdoc)) mask.push("feedFSge");
	if (updateSrcblock(dom, "DpchEngWzemCalData", "StatAppWzemCal", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWzemCalData", "StatShrWzemCal", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWzemCalData", "TagWzemCal", srcdoc)) mask.push("tag");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWzemCalData") {
				mergeDpchEngData(dom);

				// establish sub-panel scrJref's as global variables, and start sub-panel load sequence
				scrJrefHeadbar = retrieveSi(srcdoc, "StatShrWzemCal", "scrJrefHeadbar");
				scrJrefList = retrieveSi(srcdoc, "StatShrWzemCal", "scrJrefList");
				scrJrefRec = retrieveSi(srcdoc, "StatShrWzemCal", "scrJrefRec");
		...
		if (blk) {
			...
			if (_scrJref != "") {
				if (_scrJref == scrJref) {
					...
					} else if (blk.nodeName == "DpchEngWzemCalData") {
						mergeDpchEngData(dom);
						refresh();
					};
...
function restartNotify() {
	var srefIxWzemVReqitmode = retrieveSi(srcdoc, "StatAppWzemCal", "srefIxWzemVReqitmode");

	if (srefIxWzemVReqitmode != "notify") {
		if (doc.timeout) window.clearTimeout(doc.timeout);

		srefIxWzemVReqitmode = "notify";
		setSi(srcdoc, "StatAppWzemCal", "srefIxWzemVReqitmode", srefIxWzemVReqitmode);
...
function iterateReqit(ev) {
	...
	var srefIxWzemVReqitmode = retrieveSi(srcdoc, "StatAppWzemCal", "srefIxWzemVReqitmode");
	var latency = parseInt(retrieveSi(srcdoc, "StatAppWzemCal", "latency"));
