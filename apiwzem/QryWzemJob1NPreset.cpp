/**
	...
	* API code for job QryWzemJob1NPreset (implementation)
...
QryWzemJob1NPreset::StatApp::StatApp(
			const uint firstcol
...
bool QryWzemJob1NPreset::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWzemJob1NPreset");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryWzemJob1NPreset";
...
set<uint> QryWzemJob1NPreset::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWzemJob1NPreset::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWzemJob1NPreset::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWzemJob1NPreset");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryWzemJob1NPreset";
