/**
	...
	* API code for job PnlWzemPrsRec (implementation)
...
bool PnlWzemPrsRec::ContInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfWzemPrsRec");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemInfWzemPrsRec";
...
bool PnlWzemPrsRec::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppWzemPrsRec");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppWzemPrsRec";
...
set<uint> PnlWzemPrsRec::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> PnlWzemPrsRec::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
PnlWzemPrsRec::StatShr::StatShr(
	...
	mask = {IXWZEMVEXPSTATE, SCRJREFDETAIL, SCRJREFADETAIL, BUTREGULARIZEACTIVE};
};

bool PnlWzemPrsRec::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrWzemPrsRec");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrWzemPrsRec";
...
set<uint> PnlWzemPrsRec::StatShr::diff(
	...
	diffitems = {IXWZEMVEXPSTATE, SCRJREFDETAIL, SCRJREFADETAIL, BUTREGULARIZEACTIVE};
	for (auto it = commitems.begin(); it != commitems.end(); it++) diffitems.erase(*it);
...
bool PnlWzemPrsRec::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagWzemPrsRec");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemWzemPrsRec";
...
PnlWzemPrsRec::DpchAppDo::DpchAppDo(
			...
			DpchAppWzem(VecWzemVDpch::DPCHAPPWZEMPRSRECDO, scrJref)
		{
...
void PnlWzemPrsRec::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppWzemPrsRecDo");
...
PnlWzemPrsRec::DpchEngData::DpchEngData() :
			DpchEngWzem(VecWzemVDpch::DPCHENGWZEMPRSRECDATA)
		{
};
...
void PnlWzemPrsRec::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngWzemPrsRecData");
	else
		basefound = checkXPath(docctx, basexpath);
