/**
	...
	* API code for job PnlWzemClnList (implementation)
...
bool PnlWzemClnList::ContInf::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfWzemClnList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemInfWzemClnList";
...
bool PnlWzemClnList::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrWzemClnList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrWzemClnList";
...
PnlWzemClnList::Tag::Tag(
	...
	mask = {CPT, TXTFOR, TXTRECORD1, TXTRECORD2, TRS, TXTSHOWING1, TXTSHOWING2, TCOJOB, TCOCAL, TCOSGE, TCOSTA, TCOSTO};
};

bool PnlWzemClnList::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagWzemClnList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemWzemClnList";
	...
	if (basefound) {
		...
		if (extractStringAttrUclc(docctx, basexpath, itemtag, "Ti", "sref", "TcoSge", TcoSge)) add(TCOSGE);
...
PnlWzemClnList::DpchAppData::DpchAppData(
			...
			DpchAppWzem(VecWzemVDpch::DPCHAPPWZEMCLNLISTDATA, scrJref)
		{
...
void PnlWzemClnList::DpchAppData::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppWzemClnListData");
...
PnlWzemClnList::DpchAppDo::DpchAppDo(
			...
			DpchAppWzem(VecWzemVDpch::DPCHAPPWZEMCLNLISTDO, scrJref)
		{
...
void PnlWzemClnList::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppWzemClnListDo");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/wzem");
		if (has(SCRJREF)) writeString(wr, "scrJref", scrJref);
...
PnlWzemClnList::DpchEngData::DpchEngData() :
			DpchEngWzem(VecWzemVDpch::DPCHENGWZEMCLNLISTDATA)
		{
...
void PnlWzemClnList::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngWzemClnListData");
	else
		basefound = checkXPath(docctx, basexpath);
	...
	if (basefound) {
		...
		tag = Tag();
		rst.clear();
		statappqry = QryWzemClnList::StatApp();
		statshrqry = QryWzemClnList::StatShr();
