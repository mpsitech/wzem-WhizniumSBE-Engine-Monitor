/**
	...
	* API code for job QryWzemJob1NClstn (implementation)
...
QryWzemJob1NClstn::StatApp::StatApp(
			const uint firstcol
...
bool QryWzemJob1NClstn::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWzemJob1NClstn");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryWzemJob1NClstn";
...
set<uint> QryWzemJob1NClstn::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWzemJob1NClstn::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWzemJob1NClstn::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWzemJob1NClstn");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryWzemJob1NClstn";
