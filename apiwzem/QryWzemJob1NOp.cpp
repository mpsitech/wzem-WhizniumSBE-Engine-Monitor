/**
	...
	* API code for job QryWzemJob1NOp (implementation)
...
QryWzemJob1NOp::StatApp::StatApp(
			const uint firstcol
...
bool QryWzemJob1NOp::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWzemJob1NOp");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryWzemJob1NOp";
...
set<uint> QryWzemJob1NOp::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWzemJob1NOp::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWzemJob1NOp::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWzemJob1NOp");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryWzemJob1NOp";
