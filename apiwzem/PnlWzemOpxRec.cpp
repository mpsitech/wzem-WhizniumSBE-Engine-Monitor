/**
	...
	* API code for job PnlWzemOpxRec (implementation)
...
bool PnlWzemOpxRec::ContInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfWzemOpxRec");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemInfWzemOpxRec";
...
bool PnlWzemOpxRec::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppWzemOpxRec");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppWzemOpxRec";
...
set<uint> PnlWzemOpxRec::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> PnlWzemOpxRec::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool PnlWzemOpxRec::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrWzemOpxRec");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrWzemOpxRec";
...
bool PnlWzemOpxRec::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagWzemOpxRec");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemWzemOpxRec";
...
PnlWzemOpxRec::DpchAppDo::DpchAppDo(
			...
			DpchAppWzem(VecWzemVDpch::DPCHAPPWZEMOPXRECDO, scrJref)
		{
...
void PnlWzemOpxRec::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppWzemOpxRecDo");
...
PnlWzemOpxRec::DpchEngData::DpchEngData() :
			DpchEngWzem(VecWzemVDpch::DPCHENGWZEMOPXRECDATA)
		{
};
...
void PnlWzemOpxRec::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngWzemOpxRecData");
	else
		basefound = checkXPath(docctx, basexpath);
