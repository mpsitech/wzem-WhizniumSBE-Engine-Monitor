/**
	...
	* API code for job QryWzemPrd1NEvent (implementation)
...
QryWzemPrd1NEvent::StatApp::StatApp(
			const uint firstcol
...
bool QryWzemPrd1NEvent::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWzemPrd1NEvent");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryWzemPrd1NEvent";
...
set<uint> QryWzemPrd1NEvent::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWzemPrd1NEvent::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWzemPrd1NEvent::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWzemPrd1NEvent");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryWzemPrd1NEvent";
