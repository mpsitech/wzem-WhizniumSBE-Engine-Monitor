/**
	...
	* API code for job PnlWzemOpxHeadbar (implementation)
...
PnlWzemOpxHeadbar::StgInf::StgInf(
...
bool PnlWzemOpxHeadbar::StgInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgInfWzemOpxHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemInfWzemOpxHeadbar";
...
set<uint> PnlWzemOpxHeadbar::StgInf::comm(
			const StgInf* comp
		) {
	set<uint> items;
...
set<uint> PnlWzemOpxHeadbar::StgInf::diff(
			const StgInf* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool PnlWzemOpxHeadbar::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagWzemOpxHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemWzemOpxHeadbar";
...
PnlWzemOpxHeadbar::DpchEngData::DpchEngData() :
			DpchEngWzem(VecWzemVDpch::DPCHENGWZEMOPXHEADBARDATA)
		{
};
...
void PnlWzemOpxHeadbar::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngWzemOpxHeadbarData");
	else
		basefound = checkXPath(docctx, basexpath);
