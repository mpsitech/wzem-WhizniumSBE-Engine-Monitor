...
QryWzemClnList::StatApp::StatApp(
			const uint firstcol
...
bool QryWzemClnList::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWzemClnList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryWzemClnList";
...
set<uint> QryWzemClnList::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWzemClnList::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWzemClnList::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWzemClnList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryWzemClnList";
