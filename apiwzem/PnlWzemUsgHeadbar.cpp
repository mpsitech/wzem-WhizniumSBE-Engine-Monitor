/**
	* \file PnlWzemUsgHeadbar.cpp
	* API code for job PnlWzemUsgHeadbar (implementation)
...
PnlWzemUsgHeadbar::StgInf::StgInf(
...
bool PnlWzemUsgHeadbar::StgInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgInfWzemUsgHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemInfWzemUsgHeadbar";
...
set<uint> PnlWzemUsgHeadbar::StgInf::comm(
			const StgInf* comp
		) {
	set<uint> items;
...
set<uint> PnlWzemUsgHeadbar::StgInf::diff(
			const StgInf* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool PnlWzemUsgHeadbar::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagWzemUsgHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemWzemUsgHeadbar";
...
PnlWzemUsgHeadbar::DpchEngData::DpchEngData() :
			DpchEngWzem(VecWzemVDpch::DPCHENGWZEMUSGHEADBARDATA)
		{
};
...
void PnlWzemUsgHeadbar::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngWzemUsgHeadbarData");
	else
		basefound = checkXPath(docctx, basexpath);
