/**
	...
	* API code for job PnlWzemEvtList (implementation)
...
bool PnlWzemEvtList::ContInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfWzemEvtList");
	else
		basefound = checkXPath(docctx, basexpath);
...
bool PnlWzemEvtList::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrWzemEvtList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrWzemEvtList";
...
PnlWzemEvtList::Tag::Tag(
	...
	mask = {CPT, TRS, TXTSHOWING1, TXTSHOWING2, TCOTYP, TCOPRD, TCOSTA, TCOCMD, TCOFEG, TCOMTD, TCOXSR};
};
...
bool PnlWzemEvtList::Tag::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagWzemEvtList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemWzemEvtList";
	...
	if (basefound) {
		...
		if (extractStringAttrUclc(docctx, basexpath, itemtag, "Ti", "sref", "TcoCmd", TcoCmd)) add(TCOCMD);
		if (extractStringAttrUclc(docctx, basexpath, itemtag, "Ti", "sref", "TcoFeg", TcoFeg)) add(TCOFEG);
		if (extractStringAttrUclc(docctx, basexpath, itemtag, "Ti", "sref", "TcoMtd", TcoMtd)) add(TCOMTD);
...
PnlWzemEvtList::DpchAppData::DpchAppData(
			...
			DpchAppWzem(VecWzemVDpch::DPCHAPPWZEMEVTLISTDATA, scrJref)
		{
...
void PnlWzemEvtList::DpchAppData::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppWzemEvtListData");
...
PnlWzemEvtList::DpchAppDo::DpchAppDo(
			...
			DpchAppWzem(VecWzemVDpch::DPCHAPPWZEMEVTLISTDO, scrJref)
		{
...
void PnlWzemEvtList::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppWzemEvtListDo");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/wzem");
		if (has(SCRJREF)) writeString(wr, "scrJref", scrJref);
...
PnlWzemEvtList::DpchEngData::DpchEngData() :
			DpchEngWzem(VecWzemVDpch::DPCHENGWZEMEVTLISTDATA)
		{
...
void PnlWzemEvtList::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngWzemEvtListData");
	else
		basefound = checkXPath(docctx, basexpath);
	...
	if (basefound) {
		...
		tag = Tag();
		rst.clear();
		statappqry = QryWzemEvtList::StatApp();
		statshrqry = QryWzemEvtList::StatShr();
