/**
	...
	* API code for job QryWzemPrd1NNode (implementation)
...
QryWzemPrd1NNode::StatApp::StatApp(
			const uint firstcol
...
bool QryWzemPrd1NNode::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWzemPrd1NNode");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryWzemPrd1NNode";
...
set<uint> QryWzemPrd1NNode::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWzemPrd1NNode::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWzemPrd1NNode::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWzemPrd1NNode");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryWzemPrd1NNode";
