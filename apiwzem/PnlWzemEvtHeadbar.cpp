/**
	* \file PnlWzemEvtHeadbar.cpp
	* API code for job PnlWzemEvtHeadbar (implementation)
...
PnlWzemEvtHeadbar::StgInf::StgInf(
...
bool PnlWzemEvtHeadbar::StgInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgInfWzemEvtHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemInfWzemEvtHeadbar";
...
set<uint> PnlWzemEvtHeadbar::StgInf::comm(
			const StgInf* comp
		) {
	set<uint> items;
...
set<uint> PnlWzemEvtHeadbar::StgInf::diff(
			const StgInf* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool PnlWzemEvtHeadbar::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagWzemEvtHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemWzemEvtHeadbar";
...
PnlWzemEvtHeadbar::DpchEngData::DpchEngData() :
			DpchEngWzem(VecWzemVDpch::DPCHENGWZEMEVTHEADBARDATA)
		{
};
...
void PnlWzemEvtHeadbar::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngWzemEvtHeadbarData");
	else
		basefound = checkXPath(docctx, basexpath);
