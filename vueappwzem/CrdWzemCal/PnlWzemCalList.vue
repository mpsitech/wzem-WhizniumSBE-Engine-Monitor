...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWzemCalList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWzemCalListData": {
						"scrJref": this.scrJref,
						"StgIacQryWzemCalList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWzemCalList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWzemCalListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWzemCalListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacWzemCalList) this.contiac = dpcheng.ContIacWzemCalList;
				if (dpcheng.ContInfWzemCalList) this.continf = dpcheng.ContInfWzemCalList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrWzemCalList) this.statshr = dpcheng.StatShrWzemCalList;
				if (dpcheng.StgIacWzemCalList) this.stgiac = dpcheng.StgIacWzemCalList;
				if (dpcheng.TagWzemCalList) {
					Wzem.unescapeBlock(dpcheng.TagWzemCalList);
					this.tag = dpcheng.TagWzemCalList;
				}
				if (dpcheng.ListWzemQCalList) this.rst = dpcheng.ListWzemQCalList;
				if (dpcheng.StatAppQryWzemCalList) this.statappqry = dpcheng.StatAppQryWzemCalList;
				if (dpcheng.StatShrQryWzemCalList) this.statshrqry = dpcheng.StatShrQryWzemCalList;
				if (dpcheng.StgIacQryWzemCalList) this.stgiacqry = dpcheng.StgIacQryWzemCalList;
				if (dpcheng.ContInfWzemCalList) {
				...
				*/
				...
				if (dpcheng.TagWzemCalList) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoJob", text: this.tag.TcoJob}, {value: "TcoCal", text: this.tag.TcoCal}, {value: "TcoSta", text: this.tag.TcoSta}, {value: "TcoSto", text: this.tag.TcoSto});
					/*
					*/
				...
				if (dpcheng.ListWzemQCalList) {
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoJob: rec["job"], TcoCal: rec["cal"], TcoSta: rec["sta"], TcoSto: rec["sto"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxWzemVDpch, dpcheng) {
				if (srefIxWzemVDpch == "DpchEngWzemCalListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWzemVDpch == "DpchEngWzemCalListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
