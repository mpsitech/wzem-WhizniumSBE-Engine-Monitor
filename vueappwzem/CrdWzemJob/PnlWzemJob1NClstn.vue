...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWzemJob1NClstn.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWzemJob1NClstnData": {
						"scrJref": this.scrJref,
						"StgIacQryWzemJob1NClstn": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWzemJob1NClstn"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWzemJob1NClstnData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWzemJob1NClstnData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfWzemJob1NClstn) this.continf = dpcheng.ContInfWzemJob1NClstn;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppWzemJob1NClstn) this.statapp = dpcheng.StatAppWzemJob1NClstn;
				if (dpcheng.StatShrWzemJob1NClstn) this.statshr = dpcheng.StatShrWzemJob1NClstn;
				if (dpcheng.StgIacWzemJob1NClstn) this.stgiac = dpcheng.StgIacWzemJob1NClstn;
				if (dpcheng.TagWzemJob1NClstn) {
					Wzem.unescapeBlock(dpcheng.TagWzemJob1NClstn);
					this.tag = dpcheng.TagWzemJob1NClstn;
				}
				if (dpcheng.ListWzemQJob1NClstn) this.rst = dpcheng.ListWzemQJob1NClstn;
				if (dpcheng.StatAppQryWzemJob1NClstn) this.statappqry = dpcheng.StatAppQryWzemJob1NClstn;
				if (dpcheng.StatShrQryWzemJob1NClstn) this.statshrqry = dpcheng.StatShrQryWzemJob1NClstn;
				if (dpcheng.StgIacQryWzemJob1NClstn) this.stgiacqry = dpcheng.StgIacQryWzemJob1NClstn;
				/*
				...
				*/
				...
				if (dpcheng.ListWzemQJob1NClstn) {
					var rows = [];
			...
			handleDpchAppDataDoReply: function(srefIxWzemVDpch, dpcheng) {
				if (srefIxWzemVDpch == "DpchEngWzemJob1NClstnData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWzemVDpch == "DpchEngWzemJob1NClstnData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
