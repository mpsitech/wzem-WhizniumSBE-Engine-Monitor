...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWzemJob1NOp.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWzemJob1NOpData": {
						"scrJref": this.scrJref,
						"StgIacQryWzemJob1NOp": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWzemJob1NOp"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWzemJob1NOpData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWzemJob1NOpData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfWzemJob1NOp) this.continf = dpcheng.ContInfWzemJob1NOp;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppWzemJob1NOp) this.statapp = dpcheng.StatAppWzemJob1NOp;
				if (dpcheng.StatShrWzemJob1NOp) this.statshr = dpcheng.StatShrWzemJob1NOp;
				if (dpcheng.StgIacWzemJob1NOp) this.stgiac = dpcheng.StgIacWzemJob1NOp;
				if (dpcheng.TagWzemJob1NOp) {
					Wzem.unescapeBlock(dpcheng.TagWzemJob1NOp);
					this.tag = dpcheng.TagWzemJob1NOp;
				}
				if (dpcheng.ListWzemQJob1NOp) this.rst = dpcheng.ListWzemQJob1NOp;
				if (dpcheng.StatAppQryWzemJob1NOp) this.statappqry = dpcheng.StatAppQryWzemJob1NOp;
				if (dpcheng.StatShrQryWzemJob1NOp) this.statshrqry = dpcheng.StatShrQryWzemJob1NOp;
				if (dpcheng.StgIacQryWzemJob1NOp) this.stgiacqry = dpcheng.StgIacQryWzemJob1NOp;
				/*
			...
			handleDpchAppDataDoReply: function(srefIxWzemVDpch, dpcheng) {
				if (srefIxWzemVDpch == "DpchEngWzemJob1NOpData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWzemVDpch == "DpchEngWzemJob1NOpData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
