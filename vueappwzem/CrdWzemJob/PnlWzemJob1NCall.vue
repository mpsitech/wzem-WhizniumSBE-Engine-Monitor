...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWzemJob1NCall.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWzemJob1NCallData": {
						"scrJref": this.scrJref,
						"StgIacQryWzemJob1NCall": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWzemJob1NCall"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWzemJob1NCallData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWzemJob1NCallData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfWzemJob1NCall) this.continf = dpcheng.ContInfWzemJob1NCall;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppWzemJob1NCall) this.statapp = dpcheng.StatAppWzemJob1NCall;
				if (dpcheng.StatShrWzemJob1NCall) this.statshr = dpcheng.StatShrWzemJob1NCall;
				if (dpcheng.StgIacWzemJob1NCall) this.stgiac = dpcheng.StgIacWzemJob1NCall;
				if (dpcheng.TagWzemJob1NCall) {
					Wzem.unescapeBlock(dpcheng.TagWzemJob1NCall);
					this.tag = dpcheng.TagWzemJob1NCall;
				}
				if (dpcheng.ListWzemQJob1NCall) this.rst = dpcheng.ListWzemQJob1NCall;
				if (dpcheng.StatAppQryWzemJob1NCall) this.statappqry = dpcheng.StatAppQryWzemJob1NCall;
				if (dpcheng.StatShrQryWzemJob1NCall) this.statshrqry = dpcheng.StatShrQryWzemJob1NCall;
				if (dpcheng.StgIacQryWzemJob1NCall) this.stgiacqry = dpcheng.StgIacQryWzemJob1NCall;
				/*
			...
			handleDpchAppDataDoReply: function(srefIxWzemVDpch, dpcheng) {
				if (srefIxWzemVDpch == "DpchEngWzemJob1NCallData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWzemVDpch == "DpchEngWzemJob1NCallData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
