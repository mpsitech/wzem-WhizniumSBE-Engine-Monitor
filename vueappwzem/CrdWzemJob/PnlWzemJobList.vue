...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWzemJobList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWzemJobListData": {
						"scrJref": this.scrJref,
						"StgIacQryWzemJobList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWzemJobList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWzemJobListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWzemJobListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacWzemJobList) this.contiac = dpcheng.ContIacWzemJobList;
				if (dpcheng.ContInfWzemJobList) this.continf = dpcheng.ContInfWzemJobList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrWzemJobList) this.statshr = dpcheng.StatShrWzemJobList;
				if (dpcheng.StgIacWzemJobList) this.stgiac = dpcheng.StgIacWzemJobList;
				if (dpcheng.TagWzemJobList) {
					Wzem.unescapeBlock(dpcheng.TagWzemJobList);
					this.tag = dpcheng.TagWzemJobList;
				}
				if (dpcheng.ListWzemQJobList) this.rst = dpcheng.ListWzemQJobList;
				if (dpcheng.StatAppQryWzemJobList) this.statappqry = dpcheng.StatAppQryWzemJobList;
				if (dpcheng.StatShrQryWzemJobList) this.statshrqry = dpcheng.StatShrQryWzemJobList;
				if (dpcheng.StgIacQryWzemJobList) this.stgiacqry = dpcheng.StgIacQryWzemJobList;
				if (dpcheng.ContInfWzemJobList) {
				...
				*/
				...
				if (dpcheng.TagWzemJobList) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoPrd", text: this.tag.TcoPrd}, {value: "TcoJob", text: this.tag.TcoJob}, {value: "TcoXjr", text: this.tag.TcoXjr}, {value: "TcoSta", text: this.tag.TcoSta}, {value: "TcoSto", text: this.tag.TcoSto}, {value: "TcoSup", text: this.tag.TcoSup});
					/*
					*/
				...
				if (dpcheng.ListWzemQJobList) {
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoPrd: rec["prd"], TcoJob: rec["job"], TcoXjr: rec["xjr"], TcoSta: rec["sta"], TcoSto: rec["sto"], TcoSup: rec["sup"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxWzemVDpch, dpcheng) {
				if (srefIxWzemVDpch == "DpchEngWzemJobListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWzemVDpch == "DpchEngWzemJobListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
