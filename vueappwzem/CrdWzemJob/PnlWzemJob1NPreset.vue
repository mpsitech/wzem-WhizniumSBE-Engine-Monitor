...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWzemJob1NPreset.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWzemJob1NPresetData": {
						"scrJref": this.scrJref,
						"StgIacQryWzemJob1NPreset": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWzemJob1NPreset"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWzemJob1NPresetData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWzemJob1NPresetData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfWzemJob1NPreset) this.continf = dpcheng.ContInfWzemJob1NPreset;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppWzemJob1NPreset) this.statapp = dpcheng.StatAppWzemJob1NPreset;
				if (dpcheng.StatShrWzemJob1NPreset) this.statshr = dpcheng.StatShrWzemJob1NPreset;
				if (dpcheng.StgIacWzemJob1NPreset) this.stgiac = dpcheng.StgIacWzemJob1NPreset;
				if (dpcheng.TagWzemJob1NPreset) {
					Wzem.unescapeBlock(dpcheng.TagWzemJob1NPreset);
					this.tag = dpcheng.TagWzemJob1NPreset;
				}
				if (dpcheng.ListWzemQJob1NPreset) this.rst = dpcheng.ListWzemQJob1NPreset;
				if (dpcheng.StatAppQryWzemJob1NPreset) this.statappqry = dpcheng.StatAppQryWzemJob1NPreset;
				if (dpcheng.StatShrQryWzemJob1NPreset) this.statshrqry = dpcheng.StatShrQryWzemJob1NPreset;
				if (dpcheng.StgIacQryWzemJob1NPreset) this.stgiacqry = dpcheng.StgIacQryWzemJob1NPreset;
				/*
				...
				*/
				...
				if (dpcheng.ListWzemQJob1NPreset) {
					var rows = [];
			...
			handleDpchAppDataDoReply: function(srefIxWzemVDpch, dpcheng) {
				if (srefIxWzemVDpch == "DpchEngWzemJob1NPresetData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWzemVDpch == "DpchEngWzemJob1NPresetData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
