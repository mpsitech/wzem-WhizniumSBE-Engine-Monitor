...
<script>
	...
	export default {
		...
		methods: {
			...
			handleLogin: function() {
				...
				dpch.post('', data).then(function (response) {
					const dpcheng = response.data.DpchEngWzemConfirm;

					if (dpcheng.accepted) {
						//console.log("CrdWzemStart.handleLogin() login accepted");
						...
						//console.log("CrdWzemStart.handleLogin() login denied");

						vm.fiFSge = getFiBySref(vm.feedFSge, "dny");
					}

				}).catch(function (error) {
					console.log("CrdWzemStart.handleLogin() error: " + error);
				});
			},
			...
			initSess: function(scrJrefSess) {
				...
				dpch.post('', data).then(function (response) {
					const dpcheng = response.data.DpchEngSessWzemData;

					vm.$emit("login", {scrJrefSess: scrJrefSess, scrJrefCrdnav: dpcheng.StatShrSessWzem.scrJrefCrdnav});

				}).catch(function (error) {
					console.log("CrdWzemStart.initSess() error: " + error);
				});
			},
			...
			handlePupLocaleChange: function() {
				...
				//console.log("CrdWzemStart.handlePupLocaleChange() new locale is " + this.fiFPupLocale.sref);
			},
		...
		computed: {
			CptVersion() {
				return(Wzem.title + " v" + Wzem.version);
			},
