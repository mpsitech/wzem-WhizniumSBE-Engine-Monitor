...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWzemUsgMNUser.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWzemUsgMNUserData": {
						"scrJref": this.scrJref,
						"StgIacQryWzemUsgMNUser": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWzemUsgMNUser"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWzemUsgMNUserData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWzemUsgMNUserData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfWzemUsgMNUser) this.continf = dpcheng.ContInfWzemUsgMNUser;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppWzemUsgMNUser) this.statapp = dpcheng.StatAppWzemUsgMNUser;
				if (dpcheng.StatShrWzemUsgMNUser) this.statshr = dpcheng.StatShrWzemUsgMNUser;
				if (dpcheng.StgIacWzemUsgMNUser) this.stgiac = dpcheng.StgIacWzemUsgMNUser;
				if (dpcheng.TagWzemUsgMNUser) {
					Wzem.unescapeBlock(dpcheng.TagWzemUsgMNUser);
					this.tag = dpcheng.TagWzemUsgMNUser;
				}
				if (dpcheng.ListWzemQUsgMNUser) this.rst = dpcheng.ListWzemQUsgMNUser;
				if (dpcheng.StatAppQryWzemUsgMNUser) this.statappqry = dpcheng.StatAppQryWzemUsgMNUser;
				if (dpcheng.StatShrQryWzemUsgMNUser) this.statshrqry = dpcheng.StatShrQryWzemUsgMNUser;
				if (dpcheng.StgIacQryWzemUsgMNUser) this.stgiacqry = dpcheng.StgIacQryWzemUsgMNUser;
				/*
			...
			handleDpchAppDataDoReply: function(srefIxWzemVDpch, dpcheng) {
				if (srefIxWzemVDpch == "DpchEngWzemUsgMNUserData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWzemVDpch == "DpchEngWzemUsgMNUserData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
