...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWzemUsgAAccess.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWzemUsgAAccessData": {
						"scrJref": this.scrJref,
						"StgIacQryWzemUsgAAccess": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWzemUsgAAccess"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWzemUsgAAccessData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWzemUsgAAccessData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfWzemUsgAAccess) this.continf = dpcheng.ContInfWzemUsgAAccess;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppWzemUsgAAccess) this.statapp = dpcheng.StatAppWzemUsgAAccess;
				if (dpcheng.StatShrWzemUsgAAccess) this.statshr = dpcheng.StatShrWzemUsgAAccess;
				if (dpcheng.StgIacWzemUsgAAccess) this.stgiac = dpcheng.StgIacWzemUsgAAccess;
				if (dpcheng.TagWzemUsgAAccess) {
					Wzem.unescapeBlock(dpcheng.TagWzemUsgAAccess);
					this.tag = dpcheng.TagWzemUsgAAccess;
				}
				if (dpcheng.ListWzemQUsgAAccess) this.rst = dpcheng.ListWzemQUsgAAccess;
				if (dpcheng.StatAppQryWzemUsgAAccess) this.statappqry = dpcheng.StatAppQryWzemUsgAAccess;
				if (dpcheng.StatShrQryWzemUsgAAccess) this.statshrqry = dpcheng.StatShrQryWzemUsgAAccess;
				if (dpcheng.StgIacQryWzemUsgAAccess) this.stgiacqry = dpcheng.StgIacQryWzemUsgAAccess;
				/*
				...
				*/
				...
				if (dpcheng.ListWzemQUsgAAccess) {
					var rows = [];
			...
			handleDpchAppDataDoReply: function(srefIxWzemVDpch, dpcheng) {
				if (srefIxWzemVDpch == "DpchEngWzemUsgAAccessData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWzemVDpch == "DpchEngWzemUsgAAccessData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
