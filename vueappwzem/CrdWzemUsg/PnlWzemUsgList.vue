...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWzemUsgList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWzemUsgListData": {
						"scrJref": this.scrJref,
						"StgIacQryWzemUsgList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWzemUsgList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWzemUsgListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWzemUsgListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacWzemUsgList) this.contiac = dpcheng.ContIacWzemUsgList;
				if (dpcheng.ContInfWzemUsgList) this.continf = dpcheng.ContInfWzemUsgList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrWzemUsgList) this.statshr = dpcheng.StatShrWzemUsgList;
				if (dpcheng.StgIacWzemUsgList) this.stgiac = dpcheng.StgIacWzemUsgList;
				if (dpcheng.TagWzemUsgList) {
					Wzem.unescapeBlock(dpcheng.TagWzemUsgList);
					this.tag = dpcheng.TagWzemUsgList;
				}
				if (dpcheng.ListWzemQUsgList) this.rst = dpcheng.ListWzemQUsgList;
				if (dpcheng.StatAppQryWzemUsgList) this.statappqry = dpcheng.StatAppQryWzemUsgList;
				if (dpcheng.StatShrQryWzemUsgList) this.statshrqry = dpcheng.StatShrQryWzemUsgList;
				if (dpcheng.StgIacQryWzemUsgList) this.stgiacqry = dpcheng.StgIacQryWzemUsgList;
				if (dpcheng.ContInfWzemUsgList) {
			...
			handleDpchAppDataDoReply: function(srefIxWzemVDpch, dpcheng) {
				if (srefIxWzemVDpch == "DpchEngWzemUsgListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWzemVDpch == "DpchEngWzemUsgListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
