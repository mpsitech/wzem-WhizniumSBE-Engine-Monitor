...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWzemPrsList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWzemPrsListData": {
						"scrJref": this.scrJref,
						"StgIacQryWzemPrsList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWzemPrsList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWzemPrsListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWzemPrsListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacWzemPrsList) this.contiac = dpcheng.ContIacWzemPrsList;
				if (dpcheng.ContInfWzemPrsList) this.continf = dpcheng.ContInfWzemPrsList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrWzemPrsList) this.statshr = dpcheng.StatShrWzemPrsList;
				if (dpcheng.StgIacWzemPrsList) this.stgiac = dpcheng.StgIacWzemPrsList;
				if (dpcheng.TagWzemPrsList) {
					Wzem.unescapeBlock(dpcheng.TagWzemPrsList);
					this.tag = dpcheng.TagWzemPrsList;
				}
				if (dpcheng.ListWzemQPrsList) this.rst = dpcheng.ListWzemQPrsList;
				if (dpcheng.StatAppQryWzemPrsList) this.statappqry = dpcheng.StatAppQryWzemPrsList;
				if (dpcheng.StatShrQryWzemPrsList) this.statshrqry = dpcheng.StatShrQryWzemPrsList;
				if (dpcheng.StgIacQryWzemPrsList) this.stgiacqry = dpcheng.StgIacQryWzemPrsList;
				if (dpcheng.ContInfWzemPrsList) {
			...
			handleDpchAppDataDoReply: function(srefIxWzemVDpch, dpcheng) {
				if (srefIxWzemVDpch == "DpchEngWzemPrsListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWzemVDpch == "DpchEngWzemPrsListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
