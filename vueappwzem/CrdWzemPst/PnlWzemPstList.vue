...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWzemPstList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWzemPstListData": {
						"scrJref": this.scrJref,
						"StgIacQryWzemPstList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWzemPstList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWzemPstListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWzemPstListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacWzemPstList) this.contiac = dpcheng.ContIacWzemPstList;
				if (dpcheng.ContInfWzemPstList) this.continf = dpcheng.ContInfWzemPstList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrWzemPstList) this.statshr = dpcheng.StatShrWzemPstList;
				if (dpcheng.StgIacWzemPstList) this.stgiac = dpcheng.StgIacWzemPstList;
				if (dpcheng.TagWzemPstList) {
					Wzem.unescapeBlock(dpcheng.TagWzemPstList);
					this.tag = dpcheng.TagWzemPstList;
				}
				if (dpcheng.ListWzemQPstList) this.rst = dpcheng.ListWzemQPstList;
				if (dpcheng.StatAppQryWzemPstList) this.statappqry = dpcheng.StatAppQryWzemPstList;
				if (dpcheng.StatShrQryWzemPstList) this.statshrqry = dpcheng.StatShrQryWzemPstList;
				if (dpcheng.StgIacQryWzemPstList) this.stgiacqry = dpcheng.StgIacQryWzemPstList;
				if (dpcheng.ContInfWzemPstList) {
				...
				*/
				...
				if (dpcheng.TagWzemPstList) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoJob", text: this.tag.TcoJob}, {value: "TcoPst", text: this.tag.TcoPst}, {value: "TcoSta", text: this.tag.TcoSta}, {value: "TcoSto", text: this.tag.TcoSto});
					/*
					*/
				...
				if (dpcheng.ListWzemQPstList) {
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoJob: rec["job"], TcoPst: rec["pst"], TcoSta: rec["sta"], TcoSto: rec["sto"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxWzemVDpch, dpcheng) {
				if (srefIxWzemVDpch == "DpchEngWzemPstListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWzemVDpch == "DpchEngWzemPstListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
