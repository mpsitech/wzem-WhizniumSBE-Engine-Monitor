...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWzemEvtList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWzemEvtListData": {
						"scrJref": this.scrJref,
						"StgIacQryWzemEvtList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWzemEvtList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWzemEvtListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWzemEvtListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacWzemEvtList) this.contiac = dpcheng.ContIacWzemEvtList;
				if (dpcheng.ContInfWzemEvtList) this.continf = dpcheng.ContInfWzemEvtList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrWzemEvtList) this.statshr = dpcheng.StatShrWzemEvtList;
				if (dpcheng.StgIacWzemEvtList) this.stgiac = dpcheng.StgIacWzemEvtList;
				if (dpcheng.TagWzemEvtList) {
					Wzem.unescapeBlock(dpcheng.TagWzemEvtList);
					this.tag = dpcheng.TagWzemEvtList;
				}
				if (dpcheng.ListWzemQEvtList) this.rst = dpcheng.ListWzemQEvtList;
				if (dpcheng.StatAppQryWzemEvtList) this.statappqry = dpcheng.StatAppQryWzemEvtList;
				if (dpcheng.StatShrQryWzemEvtList) this.statshrqry = dpcheng.StatShrQryWzemEvtList;
				if (dpcheng.StgIacQryWzemEvtList) this.stgiacqry = dpcheng.StgIacQryWzemEvtList;
				if (dpcheng.ContInfWzemEvtList) {
				...
				*/
				...
				if (dpcheng.TagWzemEvtList) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoTyp", text: this.tag.TcoTyp}, {value: "TcoPrd", text: this.tag.TcoPrd}, {value: "TcoSta", text: this.tag.TcoSta}, {value: "TcoCmd", text: this.tag.TcoCmd}, {value: "TcoFeg", text: this.tag.TcoFeg}, {value: "TcoMtd", text: this.tag.TcoMtd}, {value: "TcoXsr", text: this.tag.TcoXsr});
					/*
					*/
				...
				if (dpcheng.ListWzemQEvtList) {
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoTyp: rec["typ"], TcoPrd: rec["prd"], TcoSta: rec["sta"], TcoCmd: rec["cmd"], TcoFeg: rec["feg"], TcoMtd: rec["mtd"], TcoXsr: rec["xsr"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxWzemVDpch, dpcheng) {
				if (srefIxWzemVDpch == "DpchEngWzemEvtListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWzemVDpch == "DpchEngWzemEvtListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
