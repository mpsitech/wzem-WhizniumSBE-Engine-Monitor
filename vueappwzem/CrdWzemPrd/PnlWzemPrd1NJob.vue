...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWzemPrd1NJob.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWzemPrd1NJobData": {
						"scrJref": this.scrJref,
						"StgIacQryWzemPrd1NJob": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWzemPrd1NJob"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWzemPrd1NJobData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWzemPrd1NJobData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfWzemPrd1NJob) this.continf = dpcheng.ContInfWzemPrd1NJob;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppWzemPrd1NJob) this.statapp = dpcheng.StatAppWzemPrd1NJob;
				if (dpcheng.StatShrWzemPrd1NJob) this.statshr = dpcheng.StatShrWzemPrd1NJob;
				if (dpcheng.StgIacWzemPrd1NJob) this.stgiac = dpcheng.StgIacWzemPrd1NJob;
				if (dpcheng.TagWzemPrd1NJob) {
					Wzem.unescapeBlock(dpcheng.TagWzemPrd1NJob);
					this.tag = dpcheng.TagWzemPrd1NJob;
				}
				if (dpcheng.ListWzemQPrd1NJob) this.rst = dpcheng.ListWzemQPrd1NJob;
				if (dpcheng.StatAppQryWzemPrd1NJob) this.statappqry = dpcheng.StatAppQryWzemPrd1NJob;
				if (dpcheng.StatShrQryWzemPrd1NJob) this.statshrqry = dpcheng.StatShrQryWzemPrd1NJob;
				if (dpcheng.StgIacQryWzemPrd1NJob) this.stgiacqry = dpcheng.StgIacQryWzemPrd1NJob;
				/*
			...
			handleDpchAppDataDoReply: function(srefIxWzemVDpch, dpcheng) {
				if (srefIxWzemVDpch == "DpchEngWzemPrd1NJobData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWzemVDpch == "DpchEngWzemPrd1NJobData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
