...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWzemPrdList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWzemPrdListData": {
						"scrJref": this.scrJref,
						"StgIacQryWzemPrdList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWzemPrdList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWzemPrdListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWzemPrdListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacWzemPrdList) this.contiac = dpcheng.ContIacWzemPrdList;
				if (dpcheng.ContInfWzemPrdList) this.continf = dpcheng.ContInfWzemPrdList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrWzemPrdList) this.statshr = dpcheng.StatShrWzemPrdList;
				if (dpcheng.StgIacWzemPrdList) this.stgiac = dpcheng.StgIacWzemPrdList;
				if (dpcheng.TagWzemPrdList) {
					Wzem.unescapeBlock(dpcheng.TagWzemPrdList);
					this.tag = dpcheng.TagWzemPrdList;
				}
				if (dpcheng.ListWzemQPrdList) this.rst = dpcheng.ListWzemQPrdList;
				if (dpcheng.StatAppQryWzemPrdList) this.statappqry = dpcheng.StatAppQryWzemPrdList;
				if (dpcheng.StatShrQryWzemPrdList) this.statshrqry = dpcheng.StatShrQryWzemPrdList;
				if (dpcheng.StgIacQryWzemPrdList) this.stgiacqry = dpcheng.StgIacQryWzemPrdList;
				if (dpcheng.ContInfWzemPrdList) {
				...
				*/
				...
				if (dpcheng.TagWzemPrdList) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoGrp", text: this.tag.TcoGrp}, {value: "TcoOwn", text: this.tag.TcoOwn}, {value: "TcoVer", text: this.tag.TcoVer}, {value: "TcoSta", text: this.tag.TcoSta}, {value: "TcoSto", text: this.tag.TcoSto});
					/*
					*/
				...
				if (dpcheng.ListWzemQPrdList) {
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoGrp: rec["grp"], TcoOwn: rec["own"], TcoVer: rec["ver"], TcoSta: rec["sta"], TcoSto: rec["sto"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxWzemVDpch, dpcheng) {
				if (srefIxWzemVDpch == "DpchEngWzemPrdListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWzemVDpch == "DpchEngWzemPrdListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
