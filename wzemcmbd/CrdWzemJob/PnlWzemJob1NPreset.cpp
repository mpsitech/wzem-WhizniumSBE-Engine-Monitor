/**
	...
	* job handler for job PnlWzemJob1NPreset (implementation)
...
#include "PnlWzemJob1NPreset_blks.cpp"
#include "PnlWzemJob1NPreset_evals.cpp"
...
/******************************************************************************
 class PnlWzemJob1NPreset
 ******************************************************************************/

PnlWzemJob1NPreset::PnlWzemJob1NPreset(
			XchgWzem* xchg
			, DbsWzem* dbswzem
			, const ubigint jrefSup
			, const uint ixWzemVLocale
		) :
			JobWzem(xchg, VecWzemVJob::PNLWZEMJOB1NPRESET, jrefSup, ixWzemVLocale)
		{
	...
	qry = new QryWzemJob1NPreset(xchg, dbswzem, jref, ixWzemVLocale);
...
DpchEngWzem* PnlWzemJob1NPreset::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWzemJob1NPreset::refresh(
			DbsWzem* dbswzem
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWzemJob1NPreset::updatePreset(
			DbsWzem* dbswzem
...
void PnlWzemJob1NPreset::handleRequest(
			DbsWzem* dbswzem
			, ReqWzem* req
		) {
	if (req->ixVBasetype == ReqWzem::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWzemVDpch == VecWzemVDpch::DPCHAPPWZEMJOB1NPRESETDATA) {
		...
		} else if (req->dpchapp->ixWzemVDpch == VecWzemVDpch::DPCHAPPWZEMJOB1NPRESETDO) {
...
void PnlWzemJob1NPreset::handleDpchAppWzemInit(
			DbsWzem* dbswzem
...
void PnlWzemJob1NPreset::handleDpchAppDataStgiac(
			DbsWzem* dbswzem
			, StgIac* _stgiac
			, DpchEngWzem** dpcheng
		) {
...
void PnlWzemJob1NPreset::handleDpchAppDataStgiacqry(
			DbsWzem* dbswzem
			, QryWzemJob1NPreset::StgIac* _stgiacqry
			, DpchEngWzem** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WzemQJob1NPreset* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWzemJob1NPreset::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWzemJob1NPreset::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswzem);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWzemJob1NPreset::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recPst = WzemMPreset();

			if (recSelNew) {
				if (dbswzem->tblwzemmpreset->loadRecByRef(recSelNew->ref, &_recPst)) {
					recPst = *_recPst;
					delete _recPst;
				};
			};
...
void PnlWzemJob1NPreset::handleDpchAppDoButViewClick(
			DbsWzem* dbswzem
			, DpchEngWzem** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWzemVPreset::PREWZEMIXCRDACCPST, jref)) if (ixPre == VecWzemVPreset::PREWZEMREFPRD) {
			sref = "CrdWzemPst";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswzem, VecWzemVCall::CALLWZEMCRDOPEN, jref, ixPre, refPre, sref, recPst.ref, jrefNew);
		};
...
void PnlWzemJob1NPreset::handleDpchAppDoButNewClick(
			DbsWzem* dbswzem
			, DpchEngWzem** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWzemVPreset::PREWZEMIXCRDACCPST, jref) & VecWzemWAccess::EDIT) != 0) if (ixPre == VecWzemVPreset::PREWZEMREFPRD) {
			sref = "CrdWzemPst";
...
void PnlWzemJob1NPreset::handleDpchAppDoButDeleteClick(
			DbsWzem* dbswzem
			, DpchEngWzem** dpcheng
		) {
...
void PnlWzemJob1NPreset::handleDpchAppDoButRefreshClick(
			DbsWzem* dbswzem
			, DpchEngWzem** dpcheng
		) {
	set<uint> moditems;
...
void PnlWzemJob1NPreset::handleCall(
			DbsWzem* dbswzem
			, Call* call
		) {
...
bool PnlWzemJob1NPreset::handleCallWzemStatChg(
			DbsWzem* dbswzem
			, const ubigint jrefTrig
		) {
	bool retval = false;
