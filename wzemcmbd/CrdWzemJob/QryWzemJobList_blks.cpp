/**
	* \file QryWzemJobList_blks.cpp
	* job handler for job QryWzemJobList (implementation of blocks)
...
void QryWzemJobList::VecVOrd::fillFeed(
			Feed& feed
		) {
	feed.clear();
...
void QryWzemJobList::StatApp::writeXML(
	...
	else itemtag = "StatitemAppQryWzemJobList";
...
void QryWzemJobList::StatShr::writeXML(
	...
	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrQryWzemJobList";
...
bool QryWzemJobList::StgIac::readJSON(
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["StgIacQryWzemJobList"];}();
