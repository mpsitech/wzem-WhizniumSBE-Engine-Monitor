/**
	...
	* job handler for job PnlWzemJobSup1NJob (implementation)
...
#include "PnlWzemJobSup1NJob_blks.cpp"
#include "PnlWzemJobSup1NJob_evals.cpp"
...
/******************************************************************************
 class PnlWzemJobSup1NJob
 ******************************************************************************/

PnlWzemJobSup1NJob::PnlWzemJobSup1NJob(
			XchgWzem* xchg
			, DbsWzem* dbswzem
			, const ubigint jrefSup
			, const uint ixWzemVLocale
		) :
			JobWzem(xchg, VecWzemVJob::PNLWZEMJOBSUP1NJOB, jrefSup, ixWzemVLocale)
		{
	...
	qry = new QryWzemJobSup1NJob(xchg, dbswzem, jref, ixWzemVLocale);
...
DpchEngWzem* PnlWzemJobSup1NJob::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWzemJobSup1NJob::refresh(
			DbsWzem* dbswzem
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWzemJobSup1NJob::updatePreset(
			DbsWzem* dbswzem
...
void PnlWzemJobSup1NJob::handleRequest(
			DbsWzem* dbswzem
			, ReqWzem* req
		) {
	if (req->ixVBasetype == ReqWzem::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWzemVDpch == VecWzemVDpch::DPCHAPPWZEMJOBSUP1NJOBDATA) {
		...
		} else if (req->dpchapp->ixWzemVDpch == VecWzemVDpch::DPCHAPPWZEMJOBSUP1NJOBDO) {
...
void PnlWzemJobSup1NJob::handleDpchAppWzemInit(
			DbsWzem* dbswzem
...
void PnlWzemJobSup1NJob::handleDpchAppDataStgiac(
			DbsWzem* dbswzem
			, StgIac* _stgiac
			, DpchEngWzem** dpcheng
		) {
...
void PnlWzemJobSup1NJob::handleDpchAppDataStgiacqry(
			DbsWzem* dbswzem
			, QryWzemJobSup1NJob::StgIac* _stgiacqry
			, DpchEngWzem** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WzemQJobSup1NJob* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWzemJobSup1NJob::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWzemJobSup1NJob::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswzem);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWzemJobSup1NJob::StgIac::JNUM)) {
...
void PnlWzemJobSup1NJob::handleDpchAppDoButViewClick(
			DbsWzem* dbswzem
			, DpchEngWzem** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWzemVPreset::PREWZEMIXCRDACCJOB, jref)) if (ixPre == VecWzemVPreset::PREWZEMREFPRD) {
			sref = "CrdWzemJob";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswzem, VecWzemVCall::CALLWZEMCRDOPEN, jref, ixPre, refPre, sref, recJob.ref, jrefNew);
		};
...
void PnlWzemJobSup1NJob::handleDpchAppDoButNewClick(
			DbsWzem* dbswzem
			, DpchEngWzem** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWzemVPreset::PREWZEMIXCRDACCJOB, jref) & VecWzemWAccess::EDIT) != 0) if (ixPre == VecWzemVPreset::PREWZEMREFPRD) {
			sref = "CrdWzemJob";
...
void PnlWzemJobSup1NJob::handleDpchAppDoButDeleteClick(
			DbsWzem* dbswzem
			, DpchEngWzem** dpcheng
		) {
...
void PnlWzemJobSup1NJob::handleDpchAppDoButRefreshClick(
			DbsWzem* dbswzem
			, DpchEngWzem** dpcheng
		) {
	set<uint> moditems;
...
void PnlWzemJobSup1NJob::handleCall(
			DbsWzem* dbswzem
			, Call* call
		) {
...
bool PnlWzemJobSup1NJob::handleCallWzemStatChg(
			DbsWzem* dbswzem
			, const ubigint jrefTrig
		) {
	bool retval = false;
