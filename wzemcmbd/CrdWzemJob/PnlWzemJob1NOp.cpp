...
void PnlWzemJob1NOp::handleDpchAppDoButViewClick(
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWzemVPreset::PREWZEMIXCRDACCOPX, jref)) {
			sref = "CrdWzemOpx";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswzem, VecWzemVCall::CALLWZEMCRDOPEN, jref, 0, 0, sref, recOpx.ref, jrefNew);
		};
...
void PnlWzemJob1NOp::handleDpchAppDoButNewClick(
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWzemVPreset::PREWZEMIXCRDACCOPX, jref) & VecWzemWAccess::EDIT) != 0) {
			sref = "CrdWzemOpx";
