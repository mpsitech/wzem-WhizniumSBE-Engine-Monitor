...
void PnlWzemJob1NClstn::handleDpchAppDoButViewClick(
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWzemVPreset::PREWZEMIXCRDACCCLN, jref)) if (ixPre == VecWzemVPreset::PREWZEMREFPRD) {
			sref = "CrdWzemCln";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswzem, VecWzemVCall::CALLWZEMCRDOPEN, jref, ixPre, refPre, sref, recCln.ref, jrefNew);
		};
...
void PnlWzemJob1NClstn::handleDpchAppDoButNewClick(
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWzemVPreset::PREWZEMIXCRDACCCLN, jref) & VecWzemWAccess::EDIT) != 0) if (ixPre == VecWzemVPreset::PREWZEMREFPRD) {
			sref = "CrdWzemCln";
