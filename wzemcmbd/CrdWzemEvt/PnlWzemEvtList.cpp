/**
	* \file PnlWzemEvtList.cpp
	* job handler for job PnlWzemEvtList (implementation)
...
#include "PnlWzemEvtList_blks.cpp"
#include "PnlWzemEvtList_evals.cpp"
...
/******************************************************************************
 class PnlWzemEvtList
 ******************************************************************************/

PnlWzemEvtList::PnlWzemEvtList(
			XchgWzem* xchg
			, DbsWzem* dbswzem
			, const ubigint jrefSup
			, const uint ixWzemVLocale
		) :
			JobWzem(xchg, VecWzemVJob::PNLWZEMEVTLIST, jrefSup, ixWzemVLocale)
		{
	...
	QryWzemEvtList::VecVOrd::fillFeed(feedFTos);

	qry = NULL;

	// IP constructor.cust1 --- INSERT

	xchg->addRefPreset(VecWzemVPreset::PREWZEMREFSEL, jref, xchg->getRefPreset(VecWzemVPreset::PREWZEMREFEVT, jref));
	xchg->addIxPreset(VecWzemVPreset::PREWZEMIXORD, jref, QryWzemEvtList::VecVOrd::TYP);

	qry = new QryWzemEvtList(xchg, dbswzem, jref, ixWzemVLocale);

	// IP constructor.cust2 --- INSERT

	set<uint> moditems;
	statshr.ixWzemVExpstate = ((xchg->getRefPreset(VecWzemVPreset::PREWZEMREFEVT, jref)) ? VecWzemVExpstate::MIND : VecWzemVExpstate::REGD);
...
DpchEngWzem* PnlWzemEvtList::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWzemEvtList::updatePreset(
			DbsWzem* dbswzem
...
void PnlWzemEvtList::regularize(
			DbsWzem* dbswzem
			, const bool notif
			, DpchEngWzem** dpcheng
		) {
...
void PnlWzemEvtList::handleRequest(
			DbsWzem* dbswzem
			, ReqWzem* req
		) {
	if (req->ixVBasetype == ReqWzem::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWzemVDpch == VecWzemVDpch::DPCHAPPWZEMEVTLISTDATA) {
		...
		} else if (req->dpchapp->ixWzemVDpch == VecWzemVDpch::DPCHAPPWZEMEVTLISTDO) {
...
void PnlWzemEvtList::handleDpchAppWzemInit(
			DbsWzem* dbswzem
...
void PnlWzemEvtList::handleDpchAppDataContiac(
			DbsWzem* dbswzem
			, ContIac* _contiac
			, DpchEngWzem** dpcheng
		) {
	...
	if (has(diffitems, ContIac::NUMFTOS)) {
		if ((_contiac->numFTos >= QryWzemEvtList::VecVOrd::STA) && (_contiac->numFTos <= QryWzemEvtList::VecVOrd::TYP)) {
...
void PnlWzemEvtList::handleDpchAppDataStgiac(
			DbsWzem* dbswzem
			, StgIac* _stgiac
			, DpchEngWzem** dpcheng
		) {
...
void PnlWzemEvtList::handleDpchAppDataStgiacqry(
			DbsWzem* dbswzem
			, QryWzemEvtList::StgIac* _stgiacqry
			, DpchEngWzem** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWzemEvtList::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWzemEvtList::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswzem);
			refresh(dbswzem, moditems);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWzemEvtList::StgIac::JNUM)) {
			if (refSelNew == 0) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);
			xchg->triggerIxRefCall(dbswzem, VecWzemVCall::CALLWZEMREFPRESET, jref, VecWzemVPreset::PREWZEMREFEVT, refSelNew);
		};
	};
...
void PnlWzemEvtList::handleDpchAppDoButMinimizeClick(
			DbsWzem* dbswzem
			, DpchEngWzem** dpcheng
		) {
...
void PnlWzemEvtList::handleDpchAppDoButRegularizeClick(
			DbsWzem* dbswzem
			, DpchEngWzem** dpcheng
		) {
...
void PnlWzemEvtList::handleDpchAppDoButNewClick(
			DbsWzem* dbswzem
			, DpchEngWzem** dpcheng
		) {
...
void PnlWzemEvtList::handleDpchAppDoButDeleteClick(
			DbsWzem* dbswzem
			, DpchEngWzem** dpcheng
		) {
...
void PnlWzemEvtList::handleDpchAppDoButFilterClick(
			DbsWzem* dbswzem
			, DpchEngWzem** dpcheng
		) {
...
void PnlWzemEvtList::handleDpchAppDoButRefreshClick(
			DbsWzem* dbswzem
			, DpchEngWzem** dpcheng
		) {
	set<uint> moditems;
...
void PnlWzemEvtList::handleCall(
			DbsWzem* dbswzem
			, Call* call
		) {
...
bool PnlWzemEvtList::handleCallWzemStatChg(
			DbsWzem* dbswzem
			, const ubigint jrefTrig
		) {
	bool retval = false;
