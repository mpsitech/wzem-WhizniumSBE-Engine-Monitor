/**
	* \file VecWzemVError.cpp
	* vector VecWzemVError (implementation)
...
void VecWzemVError::fillFeed(
			const uint ixWzemVLocale
			, Feed& feed
		) {
	feed.clear();

	for (unsigned int i = 1; i <= 24; i++) feed.appendIxSrefTitles(i, getSref(i), getTitle(i, ixWzemVLocale));
};
