/**
	...
	* job handler for job PnlWzemPrdList (implementation)
...
#include "PnlWzemPrdList_blks.cpp"
#include "PnlWzemPrdList_evals.cpp"
...
/******************************************************************************
 class PnlWzemPrdList
 ******************************************************************************/

PnlWzemPrdList::PnlWzemPrdList(
			XchgWzem* xchg
			, DbsWzem* dbswzem
			, const ubigint jrefSup
			, const uint ixWzemVLocale
		) :
			JobWzem(xchg, VecWzemVJob::PNLWZEMPRDLIST, jrefSup, ixWzemVLocale)
		{
	...
	QryWzemPrdList::VecVOrd::fillFeed(feedFTos);

	qry = NULL;

	// IP constructor.cust1 --- INSERT

	xchg->addRefPreset(VecWzemVPreset::PREWZEMREFSEL, jref, xchg->getRefPreset(VecWzemVPreset::PREWZEMREFPRD, jref));
	xchg->addIxPreset(VecWzemVPreset::PREWZEMIXORD, jref, QryWzemPrdList::VecVOrd::STA);

	qry = new QryWzemPrdList(xchg, dbswzem, jref, ixWzemVLocale);

	// IP constructor.cust2 --- INSERT

	set<uint> moditems;
	statshr.ixWzemVExpstate = ((xchg->getRefPreset(VecWzemVPreset::PREWZEMREFPRD, jref)) ? VecWzemVExpstate::MIND : VecWzemVExpstate::REGD);
...
DpchEngWzem* PnlWzemPrdList::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWzemPrdList::updatePreset(
			DbsWzem* dbswzem
...
void PnlWzemPrdList::regularize(
			DbsWzem* dbswzem
			, const bool notif
			, DpchEngWzem** dpcheng
		) {
...
void PnlWzemPrdList::handleRequest(
			DbsWzem* dbswzem
			, ReqWzem* req
		) {
	if (req->ixVBasetype == ReqWzem::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWzemVDpch == VecWzemVDpch::DPCHAPPWZEMPRDLISTDATA) {
		...
		} else if (req->dpchapp->ixWzemVDpch == VecWzemVDpch::DPCHAPPWZEMPRDLISTDO) {
...
void PnlWzemPrdList::handleDpchAppWzemInit(
			DbsWzem* dbswzem
...
void PnlWzemPrdList::handleDpchAppDataContiac(
			DbsWzem* dbswzem
			, ContIac* _contiac
			, DpchEngWzem** dpcheng
		) {
	...
	if (has(diffitems, ContIac::NUMFTOS)) {
		if ((_contiac->numFTos >= QryWzemPrdList::VecVOrd::STA) && (_contiac->numFTos <= QryWzemPrdList::VecVOrd::OWN)) {
			muteRefresh = true;
...
void PnlWzemPrdList::handleDpchAppDataStgiac(
			DbsWzem* dbswzem
			, StgIac* _stgiac
			, DpchEngWzem** dpcheng
		) {
...
void PnlWzemPrdList::handleDpchAppDataStgiacqry(
			DbsWzem* dbswzem
			, QryWzemPrdList::StgIac* _stgiacqry
			, DpchEngWzem** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWzemPrdList::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWzemPrdList::StgIac::JNUM) || (diffitems.size() > 1)) {
			muteRefresh = true;
		...
		if (has(diffitems, QryWzemPrdList::StgIac::JNUM)) {
			if (refSelNew == 0) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);
			xchg->triggerIxRefCall(dbswzem, VecWzemVCall::CALLWZEMREFPRESET, jref, VecWzemVPreset::PREWZEMREFPRD, refSelNew);
		};
	};
...
void PnlWzemPrdList::handleDpchAppDoButMinimizeClick(
			DbsWzem* dbswzem
			, DpchEngWzem** dpcheng
		) {
...
void PnlWzemPrdList::handleDpchAppDoButRegularizeClick(
			DbsWzem* dbswzem
			, DpchEngWzem** dpcheng
		) {
...
void PnlWzemPrdList::handleDpchAppDoButNewClick(
			DbsWzem* dbswzem
			, DpchEngWzem** dpcheng
		) {
...
void PnlWzemPrdList::handleDpchAppDoButDeleteClick(
			DbsWzem* dbswzem
			, DpchEngWzem** dpcheng
		) {
...
void PnlWzemPrdList::handleDpchAppDoButFilterClick(
			DbsWzem* dbswzem
			, DpchEngWzem** dpcheng
		) {
...
void PnlWzemPrdList::handleDpchAppDoButRefreshClick(
			DbsWzem* dbswzem
			, DpchEngWzem** dpcheng
		) {
	set<uint> moditems;
...
void PnlWzemPrdList::handleCall(
			DbsWzem* dbswzem
			, Call* call
		) {
...
bool PnlWzemPrdList::handleCallWzemStatChg(
			DbsWzem* dbswzem
			, const ubigint jrefTrig
		) {
	bool retval = false;
