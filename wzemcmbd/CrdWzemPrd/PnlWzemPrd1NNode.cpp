/**
	...
	* job handler for job PnlWzemPrd1NNode (implementation)
...
#include "PnlWzemPrd1NNode_blks.cpp"
#include "PnlWzemPrd1NNode_evals.cpp"
...
/******************************************************************************
 class PnlWzemPrd1NNode
 ******************************************************************************/

PnlWzemPrd1NNode::PnlWzemPrd1NNode(
			XchgWzem* xchg
			, DbsWzem* dbswzem
			, const ubigint jrefSup
			, const uint ixWzemVLocale
		) :
			JobWzem(xchg, VecWzemVJob::PNLWZEMPRD1NNODE, jrefSup, ixWzemVLocale)
		{
	...
	qry = new QryWzemPrd1NNode(xchg, dbswzem, jref, ixWzemVLocale);
...
DpchEngWzem* PnlWzemPrd1NNode::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWzemPrd1NNode::updatePreset(
			DbsWzem* dbswzem
...
void PnlWzemPrd1NNode::handleRequest(
			DbsWzem* dbswzem
			, ReqWzem* req
		) {
	if (req->ixVBasetype == ReqWzem::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWzemVDpch == VecWzemVDpch::DPCHAPPWZEMPRD1NNODEDATA) {
		...
		} else if (req->dpchapp->ixWzemVDpch == VecWzemVDpch::DPCHAPPWZEMPRD1NNODEDO) {
...
void PnlWzemPrd1NNode::handleDpchAppWzemInit(
			DbsWzem* dbswzem
...
void PnlWzemPrd1NNode::handleDpchAppDataStgiac(
			DbsWzem* dbswzem
			, StgIac* _stgiac
			, DpchEngWzem** dpcheng
		) {
...
void PnlWzemPrd1NNode::handleDpchAppDataStgiacqry(
			DbsWzem* dbswzem
			, QryWzemPrd1NNode::StgIac* _stgiacqry
			, DpchEngWzem** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWzemPrd1NNode::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWzemPrd1NNode::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswzem);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWzemPrd1NNode::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recNde = WzemMNode();

			if (recSelNew) {
				if (dbswzem->tblwzemmnode->loadRecByRef(recSelNew->ref, &_recNde)) {
					recNde = *_recNde;
					delete _recNde;
				};
			};
...
void PnlWzemPrd1NNode::handleDpchAppDoButViewClick(
			DbsWzem* dbswzem
			, DpchEngWzem** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWzemPrd1NNode::handleDpchAppDoButNewClick(
			DbsWzem* dbswzem
			, DpchEngWzem** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWzemPrd1NNode::handleDpchAppDoButDeleteClick(
			DbsWzem* dbswzem
			, DpchEngWzem** dpcheng
		) {
...
void PnlWzemPrd1NNode::handleDpchAppDoButRefreshClick(
			DbsWzem* dbswzem
			, DpchEngWzem** dpcheng
		) {
	set<uint> moditems;
...
void PnlWzemPrd1NNode::handleCall(
			DbsWzem* dbswzem
			, Call* call
		) {
...
bool PnlWzemPrd1NNode::handleCallWzemStatChg(
			DbsWzem* dbswzem
			, const ubigint jrefTrig
		) {
	bool retval = false;
