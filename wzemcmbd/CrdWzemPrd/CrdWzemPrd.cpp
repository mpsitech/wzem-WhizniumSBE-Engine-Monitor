/**
	* \file CrdWzemPrd.cpp
	* job handler for job CrdWzemPrd (implementation)
...
CrdWzemPrd::CrdWzemPrd(
			...
			JobWzem(xchg, VecWzemVJob::CRDWZEMPRD, jrefSup, ixWzemVLocale)
		{
	...
	if ((ref + 1) != 0) xchg->triggerIxRefCall(dbswzem, VecWzemVCall::CALLWZEMREFPRESET, jref, VecWzemVPreset::PREWZEMREFPRD, ref);

	// initialize according to ref
	changeRef(dbswzem, jref, ((ref + 1) == 0) ? 0 : ref, false);

	pnllist = new PnlWzemPrdList(xchg, dbswzem, jref, ixWzemVLocale);
	pnlheadbar = new PnlWzemPrdHeadbar(xchg, dbswzem, jref, ixWzemVLocale);
	pnlrec = new PnlWzemPrdRec(xchg, dbswzem, jref, ixWzemVLocale);
...
DpchEngWzem* CrdWzemPrd::getNewDpchEng(
			set<uint> items
		) {
...
void CrdWzemPrd::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdWzemPrd/" + VecWzemVLocale::getSref(ixWzemVLocale);
	continf.MtxCrdPrd = StubWzem::getStubPrdStd(dbswzem, xchg->getRefPreset(VecWzemVPreset::PREWZEMREFPRD, jref), ixWzemVLocale, Stub::VecVNonetype::FULL);
...
void CrdWzemPrd::changeRef(
	...
	xchg->triggerIxRefToIxCall(dbswzem, VecWzemVCall::CALLWZEMRECACCESS, jref, VecWzemVCard::CRDWZEMPRD, ref, ixWzemVRecaccess);

	if (ixWzemVRecaccess != VecWzemVRecaccess::NONE) {
		xchg->addIxPreset(VecWzemVPreset::PREWZEMIXRECACC, jref, ixWzemVRecaccess);

		if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbswzem, VecWzemVCall::CALLWZEMLOG, jref, VecWzemVPreset::VOID, 0, "CrdWzemPrd", ref);
		xchg->addRefPreset(VecWzemVPreset::PREWZEMREFPRD, jref, ref);

		if (pnllist) pnllist->updatePreset(dbswzem, VecWzemVPreset::PREWZEMREFPRD, jrefTrig, notif);
		if (pnlrec) pnlrec->updatePreset(dbswzem, VecWzemVPreset::PREWZEMREFPRD, jrefTrig, notif);
...
void CrdWzemPrd::handleRequest(
	...
	if (req->ixVBasetype == ReqWzem::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWzemVDpch == VecWzemVDpch::DPCHAPPWZEMPRDDO) {
...
void CrdWzemPrd::handleDpchAppWzemInit(
			DbsWzem* dbswzem
...
void CrdWzemPrd::handleDpchAppDoClose(
			DbsWzem* dbswzem
			, DpchEngWzem** dpcheng
		) {
	muteRefresh = true;
	...
	*dpcheng = new DpchEngWzemConfirm(true, jref, "");
	xchg->triggerIxCall(dbswzem, VecWzemVCall::CALLWZEMCRDCLOSE, jref, VecWzemVCard::CRDWZEMPRD);
};

void CrdWzemPrd::handleDpchAppDoMitAppAbtClick(
			DbsWzem* dbswzem
			, DpchEngWzem** dpcheng
		) {
...
void CrdWzemPrd::handleDpchAppWzemAlert(
			DbsWzem* dbswzem
...
bool CrdWzemPrd::handleCallWzemRefPreSet(
			DbsWzem* dbswzem
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecWzemVPreset::PREWZEMREFPRD) {
...
bool CrdWzemPrd::handleCallWzemStatChg(
			DbsWzem* dbswzem
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdWzemPrd::handleCallWzemDlgClose(
			DbsWzem* dbswzem
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
void CrdWzemPrd::leaveSgeIdle(
			DbsWzem* dbswzem
		) {
	// IP leaveSgeIdle --- INSERT
};

uint CrdWzemPrd::enterSgeAlrwzemabt(
			DbsWzem* dbswzem
			, const bool reenter
		) {
...
void CrdWzemPrd::leaveSgeAlrwzemabt(
			DbsWzem* dbswzem
		) {
