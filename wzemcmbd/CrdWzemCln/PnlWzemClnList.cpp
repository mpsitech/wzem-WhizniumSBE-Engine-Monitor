...
#include "PnlWzemClnList_blks.cpp"
#include "PnlWzemClnList_evals.cpp"
...
/******************************************************************************
 class PnlWzemClnList
 ******************************************************************************/

PnlWzemClnList::PnlWzemClnList(
			XchgWzem* xchg
			, DbsWzem* dbswzem
			, const ubigint jrefSup
			, const uint ixWzemVLocale
		) :
			JobWzem(xchg, VecWzemVJob::PNLWZEMCLNLIST, jrefSup, ixWzemVLocale)
		{
	...
	QryWzemClnList::VecVOrd::fillFeed(feedFTos);

	qry = NULL;

	// IP constructor.cust1 --- INSERT

	xchg->addRefPreset(VecWzemVPreset::PREWZEMREFSEL, jref, xchg->getRefPreset(VecWzemVPreset::PREWZEMREFCLN, jref));
	xchg->addIxPreset(VecWzemVPreset::PREWZEMIXORD, jref, QryWzemClnList::VecVOrd::JOB);

	qry = new QryWzemClnList(xchg, dbswzem, jref, ixWzemVLocale);

	// IP constructor.cust2 --- INSERT

	set<uint> moditems;
	statshr.ixWzemVExpstate = ((xchg->getRefPreset(VecWzemVPreset::PREWZEMREFCLN, jref)) ? VecWzemVExpstate::MIND : VecWzemVExpstate::REGD);
...
DpchEngWzem* PnlWzemClnList::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWzemClnList::regularize(
			DbsWzem* dbswzem
			, const bool notif
			, DpchEngWzem** dpcheng
		) {
...
void PnlWzemClnList::handleRequest(
			DbsWzem* dbswzem
			, ReqWzem* req
		) {
	if (req->ixVBasetype == ReqWzem::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWzemVDpch == VecWzemVDpch::DPCHAPPWZEMCLNLISTDATA) {
		...
		} else if (req->dpchapp->ixWzemVDpch == VecWzemVDpch::DPCHAPPWZEMCLNLISTDO) {
...
void PnlWzemClnList::handleDpchAppWzemInit(
			DbsWzem* dbswzem
...
void PnlWzemClnList::handleDpchAppDataContiac(
			DbsWzem* dbswzem
			, ContIac* _contiac
			, DpchEngWzem** dpcheng
		) {
...
void PnlWzemClnList::handleDpchAppDataStgiac(
			DbsWzem* dbswzem
			, StgIac* _stgiac
			, DpchEngWzem** dpcheng
		) {
...
void PnlWzemClnList::handleDpchAppDataStgiacqry(
			DbsWzem* dbswzem
			, QryWzemClnList::StgIac* _stgiacqry
			, DpchEngWzem** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWzemClnList::StgIac::JNUM)) {
...
void PnlWzemClnList::handleDpchAppDoButMinimizeClick(
			DbsWzem* dbswzem
			, DpchEngWzem** dpcheng
		) {
...
void PnlWzemClnList::handleDpchAppDoButRegularizeClick(
			DbsWzem* dbswzem
			, DpchEngWzem** dpcheng
		) {
...
void PnlWzemClnList::handleDpchAppDoButNewClick(
			DbsWzem* dbswzem
			, DpchEngWzem** dpcheng
		) {
...
void PnlWzemClnList::handleDpchAppDoButDeleteClick(
			DbsWzem* dbswzem
			, DpchEngWzem** dpcheng
		) {
...
void PnlWzemClnList::handleDpchAppDoButFilterClick(
			DbsWzem* dbswzem
			, DpchEngWzem** dpcheng
		) {
...
void PnlWzemClnList::handleDpchAppDoButRefreshClick(
			DbsWzem* dbswzem
			, DpchEngWzem** dpcheng
		) {
	set<uint> moditems;
...
void PnlWzemClnList::handleCall(
			DbsWzem* dbswzem
			, Call* call
		) {
...
bool PnlWzemClnList::handleCallWzemStatChg(
			DbsWzem* dbswzem
			, const ubigint jrefTrig
		) {
	bool retval = false;
