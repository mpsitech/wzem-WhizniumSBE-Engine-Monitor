/**
	...
	* job handler for job PnlWzemOpxList (implementation)
...
#include "PnlWzemOpxList_blks.cpp"
#include "PnlWzemOpxList_evals.cpp"
...
/******************************************************************************
 class PnlWzemOpxList
 ******************************************************************************/

PnlWzemOpxList::PnlWzemOpxList(
			XchgWzem* xchg
			, DbsWzem* dbswzem
			, const ubigint jrefSup
			, const uint ixWzemVLocale
		) :
			JobWzem(xchg, VecWzemVJob::PNLWZEMOPXLIST, jrefSup, ixWzemVLocale)
		{
	...
	QryWzemOpxList::VecVOrd::fillFeed(feedFTos);

	qry = NULL;

	// IP constructor.cust1 --- INSERT

	xchg->addRefPreset(VecWzemVPreset::PREWZEMREFSEL, jref, xchg->getRefPreset(VecWzemVPreset::PREWZEMREFOPX, jref));
	xchg->addIxPreset(VecWzemVPreset::PREWZEMIXORD, jref, QryWzemOpxList::VecVOrd::JOB);

	qry = new QryWzemOpxList(xchg, dbswzem, jref, ixWzemVLocale);

	// IP constructor.cust2 --- INSERT

	set<uint> moditems;
	statshr.ixWzemVExpstate = ((xchg->getRefPreset(VecWzemVPreset::PREWZEMREFOPX, jref)) ? VecWzemVExpstate::MIND : VecWzemVExpstate::REGD);
...
DpchEngWzem* PnlWzemOpxList::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWzemOpxList::updatePreset(
			DbsWzem* dbswzem
...
void PnlWzemOpxList::regularize(
			DbsWzem* dbswzem
			, const bool notif
			, DpchEngWzem** dpcheng
		) {
...
void PnlWzemOpxList::handleRequest(
			DbsWzem* dbswzem
			, ReqWzem* req
		) {
	if (req->ixVBasetype == ReqWzem::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWzemVDpch == VecWzemVDpch::DPCHAPPWZEMOPXLISTDATA) {
		...
		} else if (req->dpchapp->ixWzemVDpch == VecWzemVDpch::DPCHAPPWZEMOPXLISTDO) {
...
void PnlWzemOpxList::handleDpchAppWzemInit(
			DbsWzem* dbswzem
...
void PnlWzemOpxList::handleDpchAppDataContiac(
			DbsWzem* dbswzem
			, ContIac* _contiac
			, DpchEngWzem** dpcheng
		) {
	...
	if (has(diffitems, ContIac::NUMFTOS)) {
		if ((_contiac->numFTos >= QryWzemOpxList::VecVOrd::STO) && (_contiac->numFTos <= QryWzemOpxList::VecVOrd::JOB)) {
...
void PnlWzemOpxList::handleDpchAppDataStgiac(
			DbsWzem* dbswzem
			, StgIac* _stgiac
			, DpchEngWzem** dpcheng
		) {
...
void PnlWzemOpxList::handleDpchAppDataStgiacqry(
			DbsWzem* dbswzem
			, QryWzemOpxList::StgIac* _stgiacqry
			, DpchEngWzem** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWzemOpxList::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWzemOpxList::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswzem);
			refresh(dbswzem, moditems);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWzemOpxList::StgIac::JNUM)) {
			if (refSelNew == 0) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);
			xchg->triggerIxRefCall(dbswzem, VecWzemVCall::CALLWZEMREFPRESET, jref, VecWzemVPreset::PREWZEMREFOPX, refSelNew);
		};
	};
...
void PnlWzemOpxList::handleDpchAppDoButMinimizeClick(
			DbsWzem* dbswzem
			, DpchEngWzem** dpcheng
		) {
...
void PnlWzemOpxList::handleDpchAppDoButRegularizeClick(
			DbsWzem* dbswzem
			, DpchEngWzem** dpcheng
		) {
...
void PnlWzemOpxList::handleDpchAppDoButNewClick(
			DbsWzem* dbswzem
			, DpchEngWzem** dpcheng
		) {
...
void PnlWzemOpxList::handleDpchAppDoButDeleteClick(
			DbsWzem* dbswzem
			, DpchEngWzem** dpcheng
		) {
...
void PnlWzemOpxList::handleDpchAppDoButFilterClick(
			DbsWzem* dbswzem
			, DpchEngWzem** dpcheng
		) {
...
void PnlWzemOpxList::handleDpchAppDoButRefreshClick(
			DbsWzem* dbswzem
			, DpchEngWzem** dpcheng
		) {
	set<uint> moditems;
...
void PnlWzemOpxList::handleCall(
			DbsWzem* dbswzem
			, Call* call
		) {
...
bool PnlWzemOpxList::handleCallWzemStatChg(
			DbsWzem* dbswzem
			, const ubigint jrefTrig
		) {
	bool retval = false;
