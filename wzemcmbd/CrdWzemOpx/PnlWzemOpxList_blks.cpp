/**
	* \file PnlWzemOpxList_blks.cpp
	* job handler for job PnlWzemOpxList (implementation of blocks)
...
/******************************************************************************
 class PnlWzemOpxList::VecVDo
 ******************************************************************************/

uint PnlWzemOpxList::VecVDo::getIx(
			const string& sref
		) {
...
string PnlWzemOpxList::VecVDo::getSref(
			const uint ix
		) {
...
/******************************************************************************
 class PnlWzemOpxList::ContIac
 ******************************************************************************/

PnlWzemOpxList::ContIac::ContIac(
			const uint numFTos
		) :
			Block()
		{
...
bool PnlWzemOpxList::ContIac::readJSON(
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["ContIacWzemOpxList"];}();
...
bool PnlWzemOpxList::ContIac::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContIacWzemOpxList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemIacWzemOpxList";
...
void PnlWzemOpxList::ContIac::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "ContIacWzemOpxList";
...
void PnlWzemOpxList::ContIac::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	else itemtag = "ContitemIacWzemOpxList";
...
set<uint> PnlWzemOpxList::ContIac::comm(
			const ContIac* comp
		) {
	set<uint> items;
...
set<uint> PnlWzemOpxList::ContIac::diff(
			const ContIac* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class PnlWzemOpxList::ContInf
 ******************************************************************************/

PnlWzemOpxList::ContInf::ContInf(
			const string& TxtFor
			, const string& TxtPre
...
void PnlWzemOpxList::ContInf::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "ContInfWzemOpxList";
...
void PnlWzemOpxList::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfWzemOpxList";
...
set<uint> PnlWzemOpxList::ContInf::comm(
			const ContInf* comp
		) {
	set<uint> items;
...
set<uint> PnlWzemOpxList::ContInf::diff(
			const ContInf* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class PnlWzemOpxList::StatShr
 ******************************************************************************/

PnlWzemOpxList::StatShr::StatShr(
...
void PnlWzemOpxList::StatShr::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "StatShrWzemOpxList";
...
void PnlWzemOpxList::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	else itemtag = "StatitemShrWzemOpxList";
...
set<uint> PnlWzemOpxList::StatShr::comm(
			const StatShr* comp
		) {
	set<uint> items;
...
set<uint> PnlWzemOpxList::StatShr::diff(
			const StatShr* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class PnlWzemOpxList::StgIac
 ******************************************************************************/

PnlWzemOpxList::StgIac::StgIac(
			const uint TcoJobWidth
			, const uint TcoXorWidth
	...
	this->TcoXorWidth = TcoXorWidth;
	this->TcoStaWidth = TcoStaWidth;
	this->TcoStoWidth = TcoStoWidth;
	mask = {TCOJOBWIDTH, TCOXORWIDTH, TCOSTAWIDTH, TCOSTOWIDTH};
};

bool PnlWzemOpxList::StgIac::readJSON(
			const Json::Value& sup
			, bool addbasetag
		) {
	clear();
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["StgIacWzemOpxList"];}();

	basefound = (me != Json::nullValue);

	if (basefound) {
		if (me.isMember("TcoJobWidth")) {TcoJobWidth = me["TcoJobWidth"].asUInt(); add(TCOJOBWIDTH);};
		if (me.isMember("TcoXorWidth")) {TcoXorWidth = me["TcoXorWidth"].asUInt(); add(TCOXORWIDTH);};
...
bool PnlWzemOpxList::StgIac::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgIacWzemOpxList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemIacWzemOpxList";

	if (basefound) {
		if (extractUintAttrUclc(docctx, basexpath, itemtag, "Si", "sref", "TcoJobWidth", TcoJobWidth)) add(TCOJOBWIDTH);
		if (extractUintAttrUclc(docctx, basexpath, itemtag, "Si", "sref", "TcoXorWidth", TcoXorWidth)) add(TCOXORWIDTH);
...
void PnlWzemOpxList::StgIac::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "StgIacWzemOpxList";

	Json::Value& me = sup[difftag] = Json::Value(Json::objectValue);

	me["TcoJobWidth"] = TcoJobWidth;
	me["TcoXorWidth"] = TcoXorWidth;
	me["TcoStaWidth"] = TcoStaWidth;
	me["TcoStoWidth"] = TcoStoWidth;
};

void PnlWzemOpxList::StgIac::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
...
set<uint> PnlWzemOpxList::StgIac::diff(
			const StgIac* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
	...
	diffitems = {TCOJOBWIDTH, TCOXORWIDTH, TCOSTAWIDTH, TCOSTOWIDTH};
	for (auto it = commitems.begin(); it != commitems.end(); it++) diffitems.erase(*it);
...
/******************************************************************************
 class PnlWzemOpxList::Tag
 ******************************************************************************/

void PnlWzemOpxList::Tag::writeJSON(
			const uint ixWzemVLocale
			, Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "TagWzemOpxList";
...
void PnlWzemOpxList::Tag::writeXML(
			const uint ixWzemVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
...
/******************************************************************************
 class PnlWzemOpxList::DpchAppData
 ******************************************************************************/

PnlWzemOpxList::DpchAppData::DpchAppData() :
			DpchAppWzem(VecWzemVDpch::DPCHAPPWZEMOPXLISTDATA)
		{
};

string PnlWzemOpxList::DpchAppData::getSrefsMask() {
	vector<string> ss;
	string srefs;
...
void PnlWzemOpxList::DpchAppData::readJSON(
			const Json::Value& sup
			, bool addbasetag
		) {
	clear();
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["DpchAppWzemOpxListData"];}();
	...
	if (basefound) {
		...
		contiac = ContIac();
		stgiac = StgIac();
		stgiacqry = QryWzemOpxList::StgIac();
	};
};

void PnlWzemOpxList::DpchAppData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppWzemOpxListData");
	else
		basefound = checkXPath(docctx, basexpath);
...
/******************************************************************************
 class PnlWzemOpxList::DpchAppDo
 ******************************************************************************/

PnlWzemOpxList::DpchAppDo::DpchAppDo() :
			DpchAppWzem(VecWzemVDpch::DPCHAPPWZEMOPXLISTDO)
		{
	ixVDo = 0;
};

string PnlWzemOpxList::DpchAppDo::getSrefsMask() {
	vector<string> ss;
	string srefs;
...
void PnlWzemOpxList::DpchAppDo::readJSON(
			const Json::Value& sup
			, bool addbasetag
		) {
	clear();
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["DpchAppWzemOpxListDo"];}();
...
void PnlWzemOpxList::DpchAppDo::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppWzemOpxListDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
/******************************************************************************
 class PnlWzemOpxList::DpchEngData
 ******************************************************************************/

PnlWzemOpxList::DpchEngData::DpchEngData(
			const ubigint jref
			, ContIac* contiac
			, ContInf* continf
			, Feed* feedFCsiQst
			...
			DpchEngWzem(VecWzemVDpch::DPCHENGWZEMOPXLISTDATA, jref)
		{
...
string PnlWzemOpxList::DpchEngData::getSrefsMask() {
	vector<string> ss;
	string srefs;
...
void PnlWzemOpxList::DpchEngData::merge(
			DpchEngWzem* dpcheng
		) {
...
void PnlWzemOpxList::DpchEngData::writeJSON(
			const uint ixWzemVLocale
			, Json::Value& sup
		) {
	Json::Value& me = sup["DpchEngWzemOpxListData"] = Json::Value(Json::objectValue);
	...
	if (has(STATAPPQRY)) QryWzemOpxList::StatApp::writeJSON(me);
	if (has(STATSHRQRY)) statshrqry.writeJSON(me);
	if (has(STGIACQRY)) stgiacqry.writeJSON(me);
};

void PnlWzemOpxList::DpchEngData::writeXML(
			const uint ixWzemVLocale
			, xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngWzemOpxListData");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/wzem");
		...
		if (has(STATAPPQRY)) QryWzemOpxList::StatApp::writeXML(wr);
