/**
	...
	* job handler for job PnlWzemPrsList (implementation)
...
#include "PnlWzemPrsList_blks.cpp"
#include "PnlWzemPrsList_evals.cpp"
...
/******************************************************************************
 class PnlWzemPrsList
 ******************************************************************************/

PnlWzemPrsList::PnlWzemPrsList(
			XchgWzem* xchg
			, DbsWzem* dbswzem
			, const ubigint jrefSup
			, const uint ixWzemVLocale
		) :
			JobWzem(xchg, VecWzemVJob::PNLWZEMPRSLIST, jrefSup, ixWzemVLocale)
		{
	...
	QryWzemPrsList::VecVOrd::fillFeed(feedFTos);

	qry = NULL;

	// IP constructor.cust1 --- INSERT

	xchg->addRefPreset(VecWzemVPreset::PREWZEMREFSEL, jref, xchg->getRefPreset(VecWzemVPreset::PREWZEMREFPRS, jref));
	xchg->addIxPreset(VecWzemVPreset::PREWZEMIXORD, jref, QryWzemPrsList::VecVOrd::LNM);

	qry = new QryWzemPrsList(xchg, dbswzem, jref, ixWzemVLocale);

	// IP constructor.cust2 --- INSERT

	set<uint> moditems;
	statshr.ixWzemVExpstate = ((xchg->getRefPreset(VecWzemVPreset::PREWZEMREFPRS, jref)) ? VecWzemVExpstate::MIND : VecWzemVExpstate::REGD);
...
DpchEngWzem* PnlWzemPrsList::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWzemPrsList::updatePreset(
			DbsWzem* dbswzem
...
void PnlWzemPrsList::regularize(
			DbsWzem* dbswzem
			, const bool notif
			, DpchEngWzem** dpcheng
		) {
...
void PnlWzemPrsList::handleRequest(
			DbsWzem* dbswzem
			, ReqWzem* req
		) {
	if (req->ixVBasetype == ReqWzem::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWzemVDpch == VecWzemVDpch::DPCHAPPWZEMPRSLISTDATA) {
		...
		} else if (req->dpchapp->ixWzemVDpch == VecWzemVDpch::DPCHAPPWZEMPRSLISTDO) {
...
void PnlWzemPrsList::handleDpchAppWzemInit(
			DbsWzem* dbswzem
...
void PnlWzemPrsList::handleDpchAppDataContiac(
			DbsWzem* dbswzem
			, ContIac* _contiac
			, DpchEngWzem** dpcheng
		) {
	...
	if (has(diffitems, ContIac::NUMFTOS)) {
		if ((_contiac->numFTos >= QryWzemPrsList::VecVOrd::LNM) && (_contiac->numFTos <= QryWzemPrsList::VecVOrd::GRP)) {
...
void PnlWzemPrsList::handleDpchAppDataStgiac(
			DbsWzem* dbswzem
			, StgIac* _stgiac
			, DpchEngWzem** dpcheng
		) {
...
void PnlWzemPrsList::handleDpchAppDataStgiacqry(
			DbsWzem* dbswzem
			, QryWzemPrsList::StgIac* _stgiacqry
			, DpchEngWzem** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWzemPrsList::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWzemPrsList::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswzem);
			refresh(dbswzem, moditems);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWzemPrsList::StgIac::JNUM)) {
			if (refSelNew == 0) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);
			xchg->triggerIxRefCall(dbswzem, VecWzemVCall::CALLWZEMREFPRESET, jref, VecWzemVPreset::PREWZEMREFPRS, refSelNew);
		};
	};
...
void PnlWzemPrsList::handleDpchAppDoButMinimizeClick(
			DbsWzem* dbswzem
			, DpchEngWzem** dpcheng
		) {
...
void PnlWzemPrsList::handleDpchAppDoButRegularizeClick(
			DbsWzem* dbswzem
			, DpchEngWzem** dpcheng
		) {
...
void PnlWzemPrsList::handleDpchAppDoButNewClick(
			DbsWzem* dbswzem
			, DpchEngWzem** dpcheng
		) {
...
void PnlWzemPrsList::handleDpchAppDoButDeleteClick(
			DbsWzem* dbswzem
			, DpchEngWzem** dpcheng
		) {
...
void PnlWzemPrsList::handleDpchAppDoButFilterClick(
			DbsWzem* dbswzem
			, DpchEngWzem** dpcheng
		) {
...
void PnlWzemPrsList::handleDpchAppDoButRefreshClick(
			DbsWzem* dbswzem
			, DpchEngWzem** dpcheng
		) {
	set<uint> moditems;
...
void PnlWzemPrsList::handleCall(
			DbsWzem* dbswzem
			, Call* call
		) {
...
bool PnlWzemPrsList::handleCallWzemStatChg(
			DbsWzem* dbswzem
			, const ubigint jrefTrig
		) {
	bool retval = false;
