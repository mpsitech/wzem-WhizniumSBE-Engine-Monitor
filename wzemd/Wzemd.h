/**
	* \file Wzemd.h
	* inter-thread exchange object for Wzem daemon (declarations)
...
class XchgWzemd;
typedef XchgWzemd XchgWzem;

/**
	* DpchWzemdAck (written by dOpengsrv, identified by opd_exe)
	*/
...
/**
	* DpchWzemdQuit (written by d_exe, identified by opdEngsrv)
	*/
...
/**
	* DpchWzemdReg (written by dOpengsrv, read by opd_exe)
	*/
...
/**
	* DpchWzemopdReg (written by opd_exe, read by dOpengsrv)
	*/
class DpchWzemopdReg : public DpchWzem {

public:
	static const Sbecore::uint PORT = 1;
	static const Sbecore::uint IXWZEMVOPENGTYPE = 2;
...
/**
	* DpchWzemopdUnreg (written by opd_exe, read by dOpengsrv)
	*/
class DpchWzemopdUnreg : public DpchWzem {
...
public:
	StgWzemd(const Sbecore::usmallint jobprcn = 1, const Sbecore::usmallint opengsrvport = 0, const bool appsrv = true);
...
public:
	bool readXML(xmlXPathContext* docctx, std::string basexpath = "", bool addbasetag = false);
	void writeXML(xmlTextWriter* wr, std::string difftag = "", bool shorttags = true);
	std::set<Sbecore::uint> comm(const StgWzemd* comp);
	std::set<Sbecore::uint> diff(const StgWzemd* comp);
};
...
public:
	ReqopengconWzem(const Sbecore::uint ixVState = VecVState::RECEIVE, const std::string& ip = "");
	~ReqopengconWzem();
...
public:
	NodeWzem(const Sbecore::ubigint nref, const std::string& ip, const Sbecore::uint port, const Sbecore::uint opprcn, const Sbecore::uint ixWzemVOpengtype);
	~NodeWzem();
...
public:
	...
	std::set<Sbecore::uint> icsWzemVDpch;
...
public:
	...
	// condition for operation processors (sub-threads of WzemdOpengcli)
	...
	std::map<Sbecore::ubigint,NodeWzem*> nodes;
...
public:
	...
	DpchInvWzem* pullFirstInv(NodeWzem* node);
	void returnInv(DpchInvWzem* dpchinv);
	...
	// node methods
	NodeWzem* addNode(const std::string& ip, const Sbecore::uint port, const Sbecore::uint opprcn, const Sbecore::uint ixWzemVOpengtype);
	NodeWzem* getNodeByNref(const Sbecore::ubigint nref);

	void terminateNodes();
	void terminateNodeByNref(const Sbecore::ubigint nref);

	void setNodeState(NodeWzem* node, const Sbecore::uint ixVState);
};
#endif
