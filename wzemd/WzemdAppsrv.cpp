/**
	* \file WzemdAppsrv.cpp
	* application server for Wzem daemon (implementation)
...
void WzemdAppsrv::stop(
			MHD_Daemon* d
		) {
	MHD_stop_daemon(d);
};

int WzemdAppsrv::MhdAccept(
			void* cls
			, const sockaddr* addr
			, socklen_t addrlen
		) {
...
int WzemdAppsrv::MhdCallback(
	...
	if (*con_cls == NULL) {
		...
		if (strcmp(method, "GET") == 0) {
			...
			if (s == "/") {
				ixVBasetype = ReqWzem::VecVBasetype::REDIRECT;
				...
				if (s.find("../") == string::npos) {
					...
					s2 = xchg->stgwzempath.webpath + s.substr(4);
		...
		if (ixVBasetype == ReqWzem::VecVBasetype::NONE) {
			...
			*con_cls = req;
			...
			if (req->ixVBasetype == ReqWzem::VecVBasetype::WEB) {
				...
				req->filename = xchg->stgwzempath.webpath + s.substr(4);
				...
				if (valid) {
					...
					if (dcol->dpchs.empty()) {
						if (dcol->hot) {
							...
							req->cReady.lockMutex("WzemdAppsrv", "MhdCallback[1]");
							dcol->unlockAccess("WzemdAppsrv", "MhdCallback[3]");
							req->cReady.timedwait(15000000, "WzemdAppsrv", "MhdCallback");
							// cout << "got one after timedwait" << endl;
							req->cReady.unlockMutex("WzemdAppsrv", "MhdCallback[1]");

							dcol = xchg->getDcolByJref(req->jref);
							if (dcol) {
								// note: the destructor of dcol is called after dcol is removed from the list from which it can be found by getDcolByJref()
								dcol->req = NULL;
								dcol->unlockAccess("WzemdAppsrv", "MhdCallback[4]");
						...
						dcol->hot = true;
						dcol->unlockAccess("WzemdAppsrv", "MhdCallback[5]");
				...
				if (valid) {
					...
					dcol->unlockAccess("WzemdAppsrv", "MhdCallback[6]");
				...
				if (valid) {
					...
					// wait for reply
					req->cReady.lockMutex("WzemdAppsrv", "MhdCallback[2]");
					if (req->ixVState != ReqWzem::VecVState::REPLY) req->cReady.wait("WzemdAppsrv", "MhdCallback[1]");
					req->cReady.unlockMutex("WzemdAppsrv", "MhdCallback[2]");
				...
				*upload_data_size = 0;
				...
				if (req->ixVBasetype == ReqWzem::VecVBasetype::DPCHAPP) {
					...
					if (valid) {
						...
						// wait for reply
						req->cReady.lockMutex("WzemdAppsrv", "MhdCallback[3]");
						if (req->ixVState != ReqWzem::VecVState::REPLY) req->cReady.wait("WzemdAppsrv", "MhdCallback[2]");
						req->cReady.unlockMutex("WzemdAppsrv", "MhdCallback[3]");
					...
					if (valid) {
						...
						req->cReady.lockMutex("WzemdAppsrv", "MhdCallback[4]");
						if (req->ixVState != ReqWzem::VecVState::REPLY) req->cReady.wait("WzemdAppsrv", "MhdCallback[3]");
						req->cReady.unlockMutex("WzemdAppsrv", "MhdCallback[4]");
...
#else
ssize_t WzemdAppsrv::MhdFilesend(
			void* con_cls
			, uint64_t pos
			, char* buf
			, size_t max
...
bool WzemdAppsrv::validateJrefNotify(
			XchgWzemd* xchg
			, ReqWzem* req
		) {
	bool retval = false;
	...
	if (dcol) {
		...
		dcol->unlockAccess("WzemdAppsrv", "validateJrefNotify");
	};

	return retval;
};

uint WzemdAppsrv::readDpchApp(
			XchgWzemd* xchg
			, ReqWzem* req
		) {
	uint ixWzemVDpch = 0;
